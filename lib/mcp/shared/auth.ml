...
let validate_scope metadata requested_scope =
  ...
  | Some scope ->
    ...
    let allowed_scopes = match metadata.scope with
      | None -> []
    ...
    List.iter requested_scopes ~f:(fun scope ->
      if not (Set.mem allowed_scopes_set scope) then
        raise (Invalid_scope_error (Printf.sprintf "Client was not registered with scope %s" scope))
    );
...
let validate_redirect_uri metadata redirect_uri =
  ...
  | Some uri ->
    if not (List.mem metadata.redirect_uris uri ~equal:String.equal) then
      raise (Invalid_redirect_uri_error (Printf.sprintf "Redirect URI '%s' not registered for client" uri));
    uri
  | None ->
    match metadata.redirect_uris with
    | [uri] -> uri
    | _ -> raise (Invalid_redirect_uri_error "redirect_uri must be specified when client has multiple registered URIs") 