...
module Bearer_auth_backend = struct
  ...
  let authenticate config req =
    ...
    | Some h when not (String.is_prefix ~prefix:"Bearer " (String.lowercase h)) ->
      Lwt.return None
    | Some h ->
      let token = String.drop_prefix h 7 in (* Remove "Bearer " prefix *)
...
module Require_auth_middleware = struct
  ...
  let send_auth_error ~status ~error ~description ~resource_metadata_url =
    ...
    let www_auth_parts = match resource_metadata_url with
      | None -> www_auth_parts
      | Some url -> www_auth_parts @ [Printf.sprintf "resource_metadata=\"%s\"" url]
    in
    let www_authenticate = Printf.sprintf "Bearer %s" (String.concat ~sep:", " www_auth_parts) in
    let body = `Assoc [
      ("error", `String error);
  ...
  let handle config auth_result _req next =
    ...
    | Some (auth_creds, _) ->
      let missing_scope = List.find config.required_scopes ~f:(fun required_scope ->
