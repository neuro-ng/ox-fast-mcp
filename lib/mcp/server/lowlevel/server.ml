   ...
   {[
     ...
     let main () =
       Stdio_server.with_server (fun ~read_stream ~write_stream ->
         let init_options = Server.create_initialization_options server
...
type 'lifespan_context t = {
  ...
  request_handlers: (module Mcp.Shared.Session.Request_responder.S) String.Map.t;
  notification_handlers: (Mcp.Types.client_notification -> unit Lwt.t) String.Map.t;
  notification_options: notification_options;
  tool_cache: Mcp.Types.tool String.Map.t ref;
  lifespan: (unit -> 'lifespan_context Lwt.t) * ('lifespan_context -> unit Lwt.t);
  mutable current_context: (Mcp.Types.request_id * 'lifespan_context * Mcp.Server.Session.t) option;
}

let create ?(version=None) ?(instructions=None) 
    ?(lifespan_start=fun () -> Lwt.return_unit)
    ?(lifespan_end=fun _ -> Lwt.return_unit)
    ~name () =
  ...
  { name;
    version;
    instructions;
    request_handlers = String.Map.singleton "ping" (module Ping : Mcp.Shared.Session.Request_responder.S);
...
let get_request_context t =
  ...
  | Some (request_id, lifespan_ctx, session) ->
      Some (Mcp.Shared.Context.create
        ~request_id
        ~session
...
let create_initialization_options t ?(notification_options=default_notification_options)
    ?(experimental_capabilities=String.Map.empty) () =
  let version = match t.version with
    | Some v -> v
    | None -> "unknown" (* TODO: Get from opam version *)
  in
  Mcp.Server.Models.create_initialization_options
    ~server_name:t.name
    ~server_version:version
    ~capabilities:(get_capabilities t notification_options experimental_capabilities)
...
let get_capabilities t notification_options experimental_capabilities =
  ...
  let resources_capability =
    if Map.mem t.request_handlers "list_resources" then
      Some { subscribe = false; list_changed = notification_options.resources_changed }
    else None
  in
...
let register_list_prompts t handler =
  ...
  t.request_handlers <- Map.set t.request_handlers ~key:"list_prompts"
...
let register_get_prompt t handler =
  ...
  let module H = struct
    let handle req =
      match req with
      | Get_prompt params ->
          let* result = handler ~name:params.name ?arguments:params.arguments () in
          Lwt.return (Mcp.Types.server_result_of_get_prompt result)
      | _ -> Lwt.fail_with "Invalid request type"
  end in
  t.request_handlers <- Map.set t.request_handlers ~key:"get_prompt"
    ~data:(module H : Mcp.Shared.Session.Request_responder.S)
...
let get_cached_tool_definition t tool_name =
  ...
  | None ->
      ...
      | Some (module H : Mcp.Shared.Session.Request_responder.S) ->
          Log.debug (fun m -> m "Tool cache miss for %s, refreshing cache" tool_name);
          let* result = H.handle (List_tools_request {}) in
          (match result with
           | List_tools_result { tools } ->
               t.tool_cache := List.fold tools ~init:String.Map.empty ~f:(fun acc tool ->
                 Map.set acc ~key:tool.name ~data:tool);
               Lwt.return (Map.find !(t.tool_cache) tool_name)
           | _ -> Lwt.return_none)
      | None ->
          Log.warning (fun m -> m "Tool '%s' not listed, no validation will be performed" tool_name);
          Lwt.return_none
...
let process_tool_result = function
  ...
  | `Structured content ->
      ...
      Mcp.Types.server_result_of_call_tool
        ...
        ~structured_content:(`Assoc content)
        ~is_error:false
        ()
  | `Combined (content, structured) ->
      Mcp.Types.server_result_of_call_tool
        ~content
        ~structured_content:(`Assoc structured)
        ~is_error:false
        ()
...
let register_call_tool ?(validate_input=true) t handler =
  ...
  let module H = struct
    let handle req =
      ...
      | Call_tool params ->
          ...
          let arguments = Option.value params.arguments ~default:String.Map.empty in
              ...
              | Some tool ->
                  (try
                   ...
                   with Jsonschema.Validation_error msg ->
                     Lwt.return (make_error_result ("Input validation error: " ^ msg)))
          ...
          let* result =
            try%lwt
              handler ~name:tool_name ~arguments ()
            with e ->
              Lwt.return (make_error_result (Exn.to_string e))
          in
            ...
            | Some tool when Option.is_some tool.output_schema ->
                (match result with
                 | `Structured content | `Combined (_, content) ->
                     (try
                        Jsonschema.validate (Option.value_exn tool.output_schema) (`Assoc content);
                        Lwt.return_unit
                      with Jsonschema.Validation_error msg ->
                        Lwt.return (make_error_result ("Output validation error: " ^ msg)))
                 | `Unstructured _ ->
                     Lwt.return (make_error_result
  ...
  t.request_handlers <- Map.set t.request_handlers ~key:"call_tool"
    ~data:(module H : Mcp.Shared.Session.Request_responder.S)
...
let register_progress_notification t handler =
  ...
  in
  t.notification_handlers <- Map.set t.notification_handlers ~key:"progress"
    ~data:handle

let run t ~read_stream ~write_stream ~init_options ?(raise_exceptions=false) ?(stateless=false) () =
  ...
  let rec handle_message session msg =
    match msg with
    | `Request responder ->
        let request_id = responder.Mcp.Shared.Session.Request_responder.request_id in
        ...
        let* result =
          try%lwt
            ...
            | Some (module H : Mcp.Shared.Session.Request_responder.S) ->
                H.handle req
            | None ->
                Lwt.return (Mcp.Types.error_data_of_method_not_found ())
          with e ->
    ...
    | `Notification notify ->
        (try%lwt
           match Map.find t.notification_handlers (string_of_notification_type notify) with
           | Some handler -> handler notify
           | None -> Lwt.return_unit
         with e ->
           Log.err (fun m -> m "Uncaught exception in notification handler: %s"
             (Exn.to_string e));
           Lwt.return_unit)
  ...
  let* session = Mcp.Server.Session.create
    ~read_stream
    ~write_stream
    ~init_options
    ~stateless
...
let register_list_resources t handler =
  ...
  t.request_handlers <- Map.set t.request_handlers ~key:"list_resources"
    ~data:(module H : Mcp.Shared.Session.Request_responder.S)
...
let register_list_resource_templates t handler =
  ...
  let module H = struct
    let handle _req =
      ...
      Lwt.return (Mcp.Types.server_result_of_list_resource_templates ~resource_templates:templates ())
  end in
  t.request_handlers <- Map.set t.request_handlers ~key:"list_resource_templates"
    ~data:(module H : Mcp.Shared.Session.Request_responder.S)
    ...
    let handle req =
      ...
      | Read_resource params ->
          ...
          let create_content data mime_type =
            match data with
            | `Text text ->
                Mcp.Types.text_resource_contents_of_text
                  ~uri:params.uri
                  ~text
                  ~mime_type:(Option.value mime_type ~default:"text/plain")
                  ()
            | `Blob blob ->
                Mcp.Types.blob_resource_contents_of_blob
                  ~uri:params.uri
          ...
          let contents =
            match result with
            | `Single (data, mime_type) ->
                [create_content data mime_type]
            | `Multiple contents ->
                List.map contents ~f:(fun content ->
                  create_content content.Helper_types.content content.Helper_types.mime_type)
          in
          Lwt.return (Mcp.Types.server_result_of_read_resource ~contents ())
      | _ -> Lwt.fail_with "Invalid request type"
  end in
  t.request_handlers <- Map.set t.request_handlers ~key:"read_resource"
    ~data:(module H : Mcp.Shared.Session.Request_responder.S)
...
let register_set_logging_level t handler =
  ...
  t.request_handlers <- Map.set t.request_handlers ~key:"set_level"
    ~data:(module H : Mcp.Shared.Session.Request_responder.S)
...
let register_subscribe_resource t handler =
  ...
  t.request_handlers <- Map.set t.request_handlers ~key:"subscribe"
    ~data:(module H : Mcp.Shared.Session.Request_responder.S)
...
let register_unsubscribe_resource t handler =
  ...
  t.request_handlers <- Map.set t.request_handlers ~key:"unsubscribe"
    ~data:(module H : Mcp.Shared.Session.Request_responder.S)
  ...
  let module H = struct
    let handle req =
      ...
      | Complete params ->
          ...
          let completion = Option.value completion
            ~default:{ values = []; total = None; has_more = None }
          in
          Lwt.return (Mcp.Types.server_result_of_complete ~completion ())
      | _ -> Lwt.fail_with "Invalid request type"
  end in
  t.request_handlers <- Map.set t.request_handlers ~key:"complete"
    ~data:(module H : Mcp.Shared.Session.Request_responder.S) 