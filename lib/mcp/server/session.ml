(**
   ...
   This module provides the ServerSession functionality for managing communication between the
   ...
   The ServerSession module is typically used internally by the Server module and should not
...
type t = {
  ...
  base_session: (server_request, server_notification, server_result, client_request, client_notification) Mcp.Shared.Session.Base_session.t;
  incoming_message_stream: ([ `Request of (client_request, server_result) Mcp.Shared.Session.Request_responder.t
                          | `Notification of client_notification
                          | `Error of exn ], [`Closed]) Lwt_stream.t;
  mutable message_handler: t -> [ `Request of (client_request, server_result) Mcp.Shared.Session.Request_responder.t
                               | `Notification of client_notification
...
let create ~read_stream ~write_stream ~init_options ?(stateless=false) () =
  ...
  let base_session = Mcp.Shared.Session.Base_session.create
    ~read_stream
    ~write_stream
...
let send_log_message t ~level ~data ?logger ?related_request_id () =
  let notification = server_notification_of_logging_message
    ~level
    ~data
    ?logger
    ()
  in
  Mcp.Shared.Session.Base_session.send_notification
    t.base_session
    notification
    ?related_request_id
    ()
...
let create_message t ~messages ~max_tokens ?system_prompt ?include_context
    ?temperature ?stop_sequences ?metadata ?model_preferences ?related_request_id () =
  let request = server_request_of_create_message
    ~messages
    ~max_tokens
    ?system_prompt
    ?include_context
  ...
  let metadata = Option.map related_request_id ~f:(fun id ->
    Mcp.Shared.Message.Server { related_request_id = Some id; request_context = None }
  ) in
  Mcp.Shared.Session.Base_session.send_request
    t.base_session
    request
    ?metadata
    ()
...
let elicit t ~message ~requested_schema ?related_request_id () =
  let request = server_request_of_elicit
    ~message
    ~requested_schema
    ()
  in
...
let send_progress_notification t ~progress_token ~progress ?total ?message ?related_request_id () =
  let notification = server_notification_of_progress
    ~progress_token
    ~progress
    ?total
    ?message
...
let handle_request t responder =
  ...
  | Initialize params ->
      ...
      let response = server_result_of_initialize
        ~protocol_version:(if List.mem Mcp.Shared.Version.supported_protocol_versions requested_version
                         then requested_version
                         else latest_protocol_version)
        ~capabilities:t.init_options.capabilities
        ~server_info:{ name = t.init_options.server_name;
  ...
  | _ ->
      if t.initialization_state <> Initialized then
        ...
        t.message_handler t (`Request responder)
...
let handle_message t msg =
  ...
  | `Error exn -> 
      let* () = Logs_lwt.err (fun m -> m "Error in message handler: %s" (Exn.to_string exn)) in
...
let set_message_handler t handler =
  t.message_handler <- handler
