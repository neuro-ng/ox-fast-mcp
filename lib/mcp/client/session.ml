...
and sampling_fn = request_context -> create_message_request_params -> (create_message_result, error_data) result Lwt.t

and elicitation_fn = request_context -> elicit_request_params -> (elicit_result, error_data) result Lwt.t

and list_roots_fn = request_context -> (list_roots_result, error_data) result Lwt.t
...
let create
  ...
  Lwt.return {
    ...
    elicitation_callback;
    list_roots_callback;
    logging_callback = Option.value ~default:default_logging_callback logging_callback;
    message_handler = Option.value ~default:default_message_handler message_handler;
...
let with_timeout timeout promise =
  ...
  | Some t ->
    ...
    in
    Lwt.pick [
      (let* result = promise in Lwt.return_ok result);
      timeout_promise
    ]
...
let send_request t request expected_type =
  ...
  let read_promise = 
    ...
    | Ok msg when msg.id = Some request_id ->
      begin match msg.result, msg.error with
      | Some result, None -> 
        Log.debug (fun m -> m "Received successful response for request %s" request_id);
        Lwt.return_ok result
      | None, Some error -> 
        Log.warn (fun m -> m "Request %s failed: %s" request_id error.message);
        Lwt.return_error (`RequestFailed error.message)
      | None, None -> 
        Log.warn (fun m -> m "Invalid response for request %s: no result or error" request_id);
        Lwt.return_error (`InvalidMessage "No result or error in response")
      | Some _, Some _ -> 
        Log.warn (fun m -> m "Invalid response for request %s: both result and error present" request_id);
        Lwt.return_error (`InvalidMessage "Both result and error present in response")
      end
    | Ok _ -> 
      Log.warn (fun m -> m "Received response with mismatched ID for request %s" request_id);
      Lwt.return_error (`InvalidMessage "Mismatched response ID")
    | Error e -> 
      Log.err (fun m -> m "Protocol error for request %s: %s" request_id (Printexc.to_string e));
  ...
  | Error err -> 
    Log.err (fun m -> m "Request %s failed: %s" request_id (error_to_string err));
...
let initialize t =
  let sampling = match t.sampling_callback with
    | Some _ -> Some { SamplingCapability.empty with enabled = true }
    | None -> None
  in
  let elicitation = match t.elicitation_callback with
    | Some _ -> Some { ElicitationCapability.empty with enabled = true }
    | None -> None
  in
  let roots = match t.list_roots_callback with
  ...
  if not (List.mem result.protocol_version supported_protocol_versions) then
    Lwt.fail_with ("Unsupported protocol version from server: " ^ string_of_int result.protocol_version)
  else
...
let validate_tool_result t name result =
  let rec validate_with_refresh first_try =
    ...
    | None when first_try -> 
      Log.debug (fun m -> m "No schema found for tool %s, refreshing schema cache" name);
      let* _ = list_tools t () in
      validate_with_refresh false
    | None -> 
      Log.warn (fun m -> m "Tool %s not listed by server, cannot validate any structured content" name);
      Lwt.return_unit
    | Some None -> 
      Log.debug (fun m -> m "Tool %s has no schema defined, skipping validation" name);
      Lwt.return_unit
    | Some (Some schema) ->
      match result.structured_content with
      | None -> 
        Log.err (fun m -> m "Tool %s has output schema but returned no structured content" name);
        Lwt.fail_with (Printf.sprintf "Tool %s has output schema but returned no structured content" name)
      | Some content ->
        try
          Log.debug (fun m -> m "Validating structured content for tool %s" name);
          ...
          let rec validate_schema schema value =
            match schema with
            | `Assoc props ->
              begin match List.assoc_opt "type" props with
              | Some (`String typ) ->
              ...
              begin match List.assoc_opt "properties" props, value with
              | Some (`Assoc properties), `Assoc obj ->
                List.iter (fun (key, prop_schema) ->
                  match List.assoc_opt key obj with
                  | Some v -> validate_schema prop_schema v
                  | None -> 
                    match List.assoc_opt "required" props with
                    | Some (`List required) ->
                      if List.exists (function `String k -> k = key | _ -> false) required then
                        let msg = Printf.sprintf "Missing required property %s" key in
                        Log.err (fun m -> m "Validation error for tool %s: %s" name msg);
                        raise (Invalid_argument msg)
                    | _ -> ()
                ) properties
              ...
              begin match List.assoc_opt "items" props, value with
        ...
        with Invalid_argument msg ->
          Log.err (fun m -> m "Validation error for tool %s: %s" name msg);
          Lwt.fail_with (Printf.sprintf "Invalid structured content returned by tool %s: %s" name msg)
        | e ->
          Log.err (fun m -> m "Schema validation error for tool %s: %s" name (Printexc.to_string e));
          Lwt.fail_with (Printf.sprintf "Invalid schema for tool %s: %s" name (Printexc.to_string e))
  in
...
let call_tool t name ?arguments ?read_timeout ?progress_callback () =
  ...
  let rec wait_for_response () =
    let read_promise =
      ...
      match msg with
      | Ok { id = None; params = Some notification; result = None; error = None } ->
    ...
    in
    let* result = with_timeout (Option.map float_of_int read_timeout) read_promise in
    match result with
    | Ok result -> Lwt.return result
...
let _received_request t responder =
  ...
  | ElicitRequest { params } ->
    begin match t.elicitation_callback with
    | Some callback ->
      let* response = callback ctx params in
      let client_response = match response with
        | Ok result -> ClientResult { root = result }
  ...
  | ListRootsRequest ->
    begin match t.list_roots_callback with
    | Some callback ->
  ...
  | PingRequest ->
    responder.respond (ClientResult { root = EmptyResult })
...
let _received_notification t notification =
  ...
  | LoggingMessageNotification { params } ->
    ...
    begin match notification.meta with
    | Some meta ->
      Log.debug (fun m -> m "Notification meta: source=%s target=%s timestamp=%f"
        (Option.value ~default:"unknown" meta.source)
        (Option.value ~default:"unknown" meta.target)
        meta.timestamp);
    | None ->
      Log.debug (fun m -> m "No meta information in notification");
    end;
    Lwt.return_unit
  | _ -> 
...
let rec _handle_incoming t =
  let* msg = Lwt_stream.next t.read_stream in
  match msg with
  | Ok { id = Some id; params = Some request; result = None; error = None; meta } ->
    let responder = {
      request_id = id;
      request_meta = meta;
      request = request;
      respond = (fun response -> 
        let response_meta = {
          timestamp = Unix.gettimeofday ();
          source = Some "client";
          target = Some "server";
  ...
  | Ok { id = None; params = Some notification; result = None; error = None; meta } ->
  ...
  | _ -> 
    let* () = t.message_handler (`Exception (Failure "Invalid message format")) in
    _handle_incoming t

let start t =
  if not t.initialized then
    Lwt.fail_with "Session not initialized"
  else
    _handle_incoming t 