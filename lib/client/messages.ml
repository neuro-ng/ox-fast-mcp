...
let dispatch t message =
  ...
  | `Request responder ->
      ...
      begin match responder.request.root with
      | `Ping params ->
      ...
      | `CreateMessage params ->
          ...
          responder.respond (`Empty { meta = None })
      | `Elicit params ->
          let* () = Lwt.return_unit in (* Not implemented in Python version *)
          responder.respond (`Empty { meta = None })
      end
  | `Notification notif ->
      handle_notification t notif
  | `Exception exn ->
      handle_exception t exn

let handle_message t message =
  t.message_handler t message
...
let handle_notification t = function
  ...
  | `ResourceUpdated uri -> 
      handle_resource_updated t { uri; meta = None }
  | `ResourceListChanged -> 
      handle_resource_list_changed t { meta = None }
  | `ToolListChanged -> 
      handle_tool_list_changed t { meta = None }
  | `PromptListChanged -> 
      handle_prompt_list_changed t { meta = None }

let handle_exception t exn =
  let* () = Lwt_io.printf "Exception in message handler: %s\n" (Exn.to_string exn) in
  match exn with
...
let handle_ping _t _params =
  Lwt.return_unit

let handle_list_roots _t _params =
  Lwt.return_unit

let handle_create_message _t _params =
  Lwt.return_unit
...
let handle_logging_message _t params =
  let level_str = match params.level with
    | `Debug -> "DEBUG"
    | `Info -> "INFO"
    | `Notice -> "NOTICE"
    | `Warning -> "WARNING"
    ...
    | `Emergency -> "EMERGENCY"
  in
  let logger_str = match params.logger with
...
let handle_tool_list_changed _t _params =
  Lwt_io.printf "Tool list changed\n"
...
let handle_cancelled _t params =
  let reason_str = match params.reason with
