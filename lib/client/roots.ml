...
let create_roots_callback_from_roots (roots: roots_list list) : request_context -> list_roots_result Lwt.t =
  let converted_roots = convert_roots_list roots in
  fun _context -> Lwt.return { roots = converted_roots; meta = None }

let create_roots_callback_from_fn (fn: roots_handler) : request_context -> list_roots_result Lwt.t =
  fun context ->
    let* result = 
      try%lwt
...
let create_roots_callback (handler: [< `Static of roots_list list | `Handler of roots_handler]) =
  match handler with
