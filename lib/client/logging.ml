...
let format_log_message message =
  ...
  let level = json |> member "level" |> to_string_option |> Option.value ~default:"debug" in
  let msg = json |> member "message" |> to_string_option |> Option.value ~default:"" in
  let source = json |> member "source" |> to_string_option in
  let timestamp = json |> member "timestamp" |> to_float_option in
  let formatted = match (source, timestamp) with
    | Some src, Some ts -> 
      Printf.sprintf "[%s][%f] %s: %s" src ts level msg
    | Some src, None ->
      Printf.sprintf "[%s] %s: %s" src level msg
    | None, Some ts ->
      Printf.sprintf "[%f] %s: %s" ts level msg
    | None, None ->
      Printf.sprintf "%s: %s" level msg
  in
  formatted
...
let create_log_callback ?(handler=default_log_handler) () =
  ...
  if not @@ Logs.Src.is_enabled logger then
