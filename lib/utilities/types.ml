...
let create_embedded_resource ~type_ ~resource ?(annotations=[]) () =
  { type_; resource; annotations }

let embedded_resource_to_content_type res =
  resource_content_to_content_type res.resource

let content_type_to_embedded_resource = function
  | Resource res ->
    Some {
      type_ = "resource";
      resource = res;
    ...
    module File = struct
      ...
      let get_mime_type t =
        ...
        | Some fmt ->
          ...
          if List.mem ["plain"; "txt"; "text"] fmt ~equal:String.equal
          then "text/plain"
      ...
      let to_resource_content ?mime_type ?(annotations=[]) t : resource_content =
        let data =
          match t.path, t.data with
          | Some path, None ->
            In_channel.read_all path |> Base64.encode_exn
