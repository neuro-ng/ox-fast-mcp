...
let create_exception_group message exceptions =
  { message; exceptions }

(** Iterate through nested exception groups *)
let rec iter_exc group =
  List.concat_map (fun exc ->
    match exc with
    ...
    | _ -> 
      try
        ...
        if Obj.is_block exceptions && Obj.tag exceptions = 0 then
          ...
          match exc_list with
          | nested_group when nested_group = group -> []  (* Avoid infinite recursion *)
