...
let prune_param (schema : t) (param : string) : t =
  let props = match schema |> Util.member "properties" with
    | `Assoc props -> props
    | _ -> [] in
  ...
  let new_required = match schema |> Util.member "required" with
    | `List required ->
        let filtered = List.filter (fun v -> 
          match v with 
          | `String s -> s <> param 
          | _ -> true) required in
        if filtered = [] then None
        else Some (`List filtered)
    | _ -> None in
  
  (* Construct new schema *)
  let base = match schema with
    | `Assoc fields -> fields
    | _ -> [] in
  
  let without_props_required = List.filter (fun (k, _) -> 
    k <> "properties" && k <> "required") base in
...
let prune_unused_defs (schema : t) : t =
    ...
    | `Assoc fields ->
        ...
        | Some (`String ref) ->
            ...
            | Some def_name ->
                ...
                | Some current ->
                    let refs = try Hashtbl.find referenced_by def_name
                              with Not_found -> [] in
                    Hashtbl.replace referenced_by def_name (current :: refs)
                | None ->
                    Hashtbl.replace !root_defs def_name true)
            | None -> ())
        | _ -> ());
        
        (* Walk children *)
        List.iter (fun (k, v) ->
          if not (skip_defs && k = "$defs") then
            walk ~current_def v) fields
    | `List items ->
        List.iter (walk ~current_def) items
    | _ -> () in
  ...
  | `Assoc defs ->
      ...
      List.iter (fun (def_name, value) ->
        walk ~current_def:(Some def_name) value) defs;
      ...
      let new_defs = List.filter 
        (fun (def_name, _) -> def_is_referenced def_name) defs in
      ...
      if new_defs = [] then
        ...
        `Assoc (("$defs", `Assoc new_defs) :: base)
  | _ -> schema

(** Walk the schema and optionally prune titles and additionalProperties *)
let walk_and_prune ?(prune_titles=false) ?(prune_additional_properties=false) (schema : t) : t =
  let rec walk node =
    match node with
    | `Assoc fields ->
        let fields = 
          fields 
          |> (if prune_titles then List.filter (fun (k, _) -> k <> "title") else Fun.id)
...
let prune_additional_properties (schema : t) : t =
  match schema with
  | `Assoc fields ->
      let new_fields = List.filter (fun (k, v) ->
        not (k = "additionalProperties" && v = `Bool false)) fields in
      `Assoc new_fields
  | x -> x
