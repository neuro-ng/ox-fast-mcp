...
let prune_param (schema : Yojson.Safe.t) (param : string) : Yojson.Safe.t =
  ...
  | `Assoc fields ->
    let props = 
      ...
      | Some (`Assoc props) -> 
        let new_props = List.filter props ~f:(fun (k,_) -> not (String.equal k param)) in
    ...
    let required =
      ...
      | Some (`List reqs) ->
        let new_reqs = List.filter reqs ~f:(function
          | `String s -> not (String.equal s param)
          | _ -> true)
        in
        if List.is_empty new_reqs
        then []
        else [("required", `List new_reqs)]
      | _ -> []
    in
    let other_fields = 
      ...
      (match List.Assoc.find fields ~equal:String.equal "$ref" with
       | Some (`String ref) when String.is_prefix ref ~prefix:"#/$defs/" ->
         ...
         (match current_def with
          | Some curr -> 
            Hashtbl.add_multi referenced_by ~key:def_name ~data:curr
          | None ->
            Hash_set.add root_defs def_name)
       | _ -> ());
      
      (* Walk children *)
      List.iter fields ~f:(fun (k,v) ->
        if not (skip_defs && String.equal k "$defs")
        then walk ?current_def v)
    
    | `List items ->
      List.iter items ~f:(walk ?current_def)
  ...
  (match schema with
   | `Assoc fields ->
     (match List.Assoc.find fields ~equal:String.equal "$defs" with
      | Some (`Assoc defs) ->
        List.iter defs ~f:(fun (name, value) ->
          walk ~current_def:name value)
      | _ -> ())
   | _ -> ());
  ...
  | `Assoc fields ->
    (match List.Assoc.find fields ~equal:String.equal "$defs" with
     | Some (`Assoc defs) ->
       ...
       in
       if List.is_empty new_defs
       then `Assoc (List.filter fields ~f:(fun (k,_) -> not (String.equal k "$defs")))
       else `Assoc ((List.filter fields ~f:(fun (k,_) -> not (String.equal k "$defs"))) @
                   [("$defs", `Assoc new_defs)])
     | _ -> schema)
  | other -> other

(** Walk schema and optionally prune titles and additionalProperties *)
let walk_and_prune ?(prune_titles=false) ?(prune_additional_properties=false)
  ...
  let rec walk (node : Yojson.Safe.t) =
    ...
    | `Assoc fields ->
      let fields =
        fields
        |> (if prune_titles
            then List.filter ~f:(fun (k,_) -> not (String.equal k "title"))
            else Fn.id)
        |> (if prune_additional_properties
            then List.filter ~f:(fun (k,v) ->
                not (String.equal k "additionalProperties" && equal v (`Bool false)))
            else Fn.id)
      in
      `Assoc (List.map fields ~f:(fun (k,v) -> (k, walk v)))
    | `List items ->
      `List (List.map items ~f:walk)
    | other -> other
  in
  walk schema
...
let compress_schema ?(prune_params=[]) ?(prune_defs=true)
    ...
    (schema : Yojson.Safe.t) : Yojson.Safe.t =
  let schema = schema in (* Make copy in real implementation *)
  ...
  let schema =
    if prune_titles || prune_additional_properties
    then walk_and_prune ~prune_titles ~prune_additional_properties schema
    else schema
  in
