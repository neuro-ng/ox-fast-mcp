...
module Error = struct
  ...
  let to_string = function
    ...
    | Invalid_tags tags -> sprintf "Invalid tags: [%s]" (String.concat ~sep:", " tags)
...
let yojson_of_t yojson_to_extra t =
  `Assoc [
    ("name", `String t.name);
    ("description", match t.description with None -> `Null | Some s -> `String s);
    ("tags", `List (List.map t.tags ~f:(fun s -> `String s)));
    ("enabled", `Bool t.enabled);
    ("_key", match t.key with None -> `Null | Some s -> `String s);
    ("extra", yojson_to_extra t.extra);
    ("version", match t.version with None -> `Null | Some i -> `Int i);
  ]
...
module Validation = struct
  ...
  let validate_tags tags =
    ...
    in
    if List.is_empty invalid_tags then
      Ok ()
    else
      Error (Error.Invalid_tags invalid_tags)

  let validate_version = function
    | None -> Ok ()
    | Some v when v < 0 -> Error (Error.Invalid_version "Version cannot be negative")
    | Some _ -> Ok ()
...
let convert_set_default_none (maybe_set : 'a list option) : 'a list =
  Option.value maybe_set ~default:[] |> List.dedup_and_sort ~compare:Poly.compare
...
let equal ?(compare_extra = Poly.equal) a b =
  ...
  List.equal String.equal (List.sort ~compare:String.compare a.tags) (List.sort ~compare:String.compare b.tags) &&
...
let copy ?name ?description ?tags ?enabled ?key ?extra ?version t =
  let t = {
    ...
    tags = Option.value tags ~default:t.tags |> Option.some |> convert_set_default_none;
...
let of_yojson yojson_to_extra json =
  match t_of_yojson (fun j -> match yojson_to_extra j with Ok x -> x | Error e -> raise_s [%message e]) json with
  | Error msg -> Error msg
  | Ok t ->
      match Validation.validate_all t with
      | Ok () -> Ok t
...
let hash ?(hash_extra = Hashtbl.hash) t =
  [%hash: string * string option * string list * bool * string option * int option]
    (t.name, t.description, t.tags, t.enabled, t.key, t.version) + hash_extra t.extra

(** Type conversion functions *)
let to_list ts = List.sort ts ~compare:compare_by_name
let of_list ts = List.sort ts ~compare:compare_by_name |> List.dedup_and_sort ~compare:compare_by_name
