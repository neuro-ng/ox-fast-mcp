...
(** Client state *)
type t = {
  server : Fast_mcp_server.t option;
  mutable connected : bool;
}

(** Create a new in-memory client connected to a server *)
let create_in_memory server =
  { server = Some server; connected = true }

(** Create a client (placeholder for other transport types) *)
let create () = 
  { server = None; connected = false }
...
let list_tools client =
  ...
  | Some server when client.connected -> 
    Ok (Fast_mcp_server.list_tools server)
