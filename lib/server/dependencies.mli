...
module Http_request : sig
  ...
  (** Get headers from the current HTTP request.
      @param include_all If true, include all headers. If false (default),
                        exclude problematic headers like content-length.
      @return Map of header names (lowercase) to values. Empty if no request exists. *)
