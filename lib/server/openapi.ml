...
module MCPType = struct
  type t = Tool | Resource | ResourceTemplate | Exclude [@@deriving show, eq]
...
let slugify text =
  ...
  if String.is_empty text then ""
    ...
    |> global_replace (regexp "[ \\-\\.]+")
         "_" (* Replace spaces and separators with underscore *)
    |> global_replace (regexp "[^a-zA-Z0-9_]")
         "" (* Remove non-alphanumeric except underscore *)
    |> global_replace (regexp "_+")
         "_" (* Remove multiple consecutive underscores *)
    |> String.strip ~drop:(fun c -> c = '_')
...
module OpenAPITool = struct
  ...
  let create ~client ~route ~name ~description ~parameters ?(tags = []) ?timeout
    ...
    let enhanced_description =
      format_description_with_responses ~parameters:route.parameters
        ?request_body:route.request_body ~responses:route.responses description
    in
  ...
  let run t args =
    ...
    let missing_params =
      List.filter required_path_params ~f:(fun name ->
          not (List.exists path_params ~f:(fun (n, _) -> String.equal n name)))
    in

    if not (List.is_empty missing_params) then
      Lwt.fail_with
        (sprintf "Missing required path parameters: %s"
           (String.concat ~sep:", " missing_params))
    else
      (* Replace path parameters *)
      let path =
        List.fold path_params ~init:path ~f:(fun path (name, values) ->
              ...
              Option.bind (Map.find args p.name) ~f:(fun v ->
                  if String.is_empty v then None
                    ...
                    | Ok validated ->
                      ...
                      Some (p.name, values)
                    | Error msg ->
                      None (* Log error and skip invalid parameter *))
            | _ -> None)
      ...
      let headers =
        List.filter_map t.route.parameters ~f:(fun p ->
            match p.location with
            | `Header ->
              Option.map (Map.find args p.name) ~f:(fun v -> (p.name, v))
            | _ -> None)
      in

      (* Build request body *)
      let body =
        match t.route.request_body with
        | Some rb when rb.required ->
          let body_params =
            Map.filter args ~f:(fun ~key ~data ->
                not
                  (List.exists t.route.parameters ~f:(fun p ->
                       String.equal p.name key)))
          in
          if Map.is_empty body_params then None
          else Some (Yojson.Safe.to_string (`Assoc (Map.to_alist body_params)))
        | _ -> None
      in
...
module OpenAPIResource = struct
  ...
  let create ~client ~route ~uri ~name ~description
      ?(mime_type = "application/json") ?(tags = []) ?timeout () =
    let enhanced_description =
      format_description_with_responses ~parameters:route.parameters
        ?request_body:route.request_body ~responses:route.responses description
    in
  ...
  let read t =
    ...
    | `OK ->
      let* body_str = Cohttp_lwt.Body.to_string body in
      let content_type =
        Header.get (Response.headers response) "content-type"
      in
      ...
      Lwt.return
        ...
        | Some ct
          when String.is_substring ct ~substring:"text/"
               || String.is_substring ct ~substring:"application/xml" ->
          body_str
        | _ -> body_str (* Return as-is *))
    | status ->
      let status_code = Code.code_of_status status in
      let* error_body = Cohttp_lwt.Body.to_string body in
      Lwt.fail_with (sprintf "HTTP error %d: %s" status_code error_body)
...
module OpenAPIResourceTemplate = struct
  ...
  let create ~client ~route ~uri_template ~name ~description ~parameters
      ?(tags = []) ?timeout () =
    let enhanced_description =
      format_description_with_responses ~parameters:route.parameters
        ?request_body:route.request_body ~responses:route.responses description
  ...
  let create_resource t ~uri ~params =
    let uri_parts =
      Map.to_alist params |> List.map ~f:(fun (k, v) -> sprintf "%s=%s" k v)
    in

    let description =
      Option.value t.route.description
        ~default:(sprintf "Resource for %s" t.route.path)
    in

    OpenAPIResource.create ~client:t.client ~route:t.route
      ~uri:(Uri.of_string uri)
      ~name:(sprintf "%s-%s" t.name (String.concat ~sep:"-" uri_parts))
      ~description ~tags:t.tags ?timeout:t.timeout ()
end
...
let generate_default_name route mcp_names =
  ...
  let name =
    ...
    | Some id ->
      Option.value (Map.find mcp_names id)
        ~default:
          (match String.split id ~on:'_' with
          | [ name; _ ] -> name (* Take first part before double underscore *)
          | _ -> id)
    | None ->
      Option.value route.summary
        ~default:(sprintf "%s_%s" (HttpRoute.get_method route) route.path)
  in

  let name = slugify name in

  (* Truncate to 56 chars *)
  if String.length name > 56 then String.sub name ~pos:0 ~len:56 else name
...
let get_unique_name name component_type counts =
  ...
  let updated_counts =
    match component_type with
    | `Tool ->
      { counts with tools = Map.set counts.tools ~key:name ~data:(count + 1) }
    | `Resource ->
      {
        counts with
        resources = Map.set counts.resources ~key:name ~data:(count + 1);
      }
    | `Template ->
      {
        counts with
        templates = Map.set counts.templates ~key:name ~data:(count + 1);
      }
  in

  (new_name, updated_counts)

class fast_mcp_openapi ~openapi_spec ~client ?(name = "OpenAPI FastMCP")
  ?(route_maps = default_route_mappings) ?route_map_fn ?mcp_component_fn
  ?(mcp_names = String.Map.empty) ?(tags = String.Set.empty) ?timeout () =
  object (self)
    ...
    method private create_openapi_tool route name tags =
      let open Utilities.Openapi in
      let tool_name, updated_counts =
        get_unique_name name `Tool component_counts
      in
      ...
      let description =
        ...
        | None, Some s -> s
        | None, None ->
          sprintf "Executes %s %s" (HttpRoute.get_method route) route.path
      in

      let parameters = combine_schemas route in

      let tool =
        OpenAPITool.create ~client ~route ~name:tool_name ~description
          ~parameters ~tags:(Set.to_list tags) ?timeout ()
      in
      ...
      Option.iter mcp_component_fn ~f:(fun f ->
          try
            f route tool;
            log_debug (fun m ->
                m "Tool %s customized by component_fn" tool_name)
          with e ->
            log_warning (fun m ->
                m
                  "Error in component_fn for tool %s: %s. Using component \
                   as-is."
    ...
    method private create_openapi_resource route name tags =
      let open Utilities.Openapi in
      let resource_name, updated_counts =
        get_unique_name name `Resource component_counts
      in
      component_counts <- updated_counts;

      let resource_uri =
        Uri.of_string (sprintf "resource://%s" resource_name)
      in
      ...
      let resource =
        OpenAPIResource.create ~client ~route ~uri:resource_uri
          ~name:resource_name ~description ~tags:(Set.to_list tags) ?timeout ()
      in
      ...
      Option.iter mcp_component_fn ~f:(fun f ->
          try
            f route resource;
            log_debug (fun m ->
                m "Resource %s customized by component_fn" resource_name)
          with e ->
            log_warning (fun m ->
                m
                  "Error in component_fn for resource %s: %s. Using component \
                   as-is."
    ...
    method private create_openapi_template route name tags =
      let open Utilities.Openapi in
      let template_name, updated_counts =
        get_unique_name name `Template component_counts
      in
      component_counts <- updated_counts;

      let path_params =
        List.filter route.parameters ~f:(fun p ->
            match p.location with
            | `Path -> true
            | _ -> false)
      in
      ...
      let uri_template =
        sprintf "resource://%s/%s" template_name
          (String.concat ~sep:"/"
             (List.map path_params ~f:(fun p -> sprintf "{%s}" p.name)))
      in
      ...
      let parameters =
        `Assoc
          [
            ...
            ( "properties",
              `Assoc
                (List.map path_params ~f:(fun p ->
                     ( p.name,
                       Option.value_map p.schema ~default:(`Assoc [])
                         ~f:(fun schema ->
                           `Assoc
                             (( "description",
                                `String (Option.value p.description ~default:"")
                              )
                             :: to_assoc schema)) ))) );
            ( "required",
              `List
                (List.filter_map path_params ~f:(fun p ->
                     if p.required then Some (`String p.name) else None)) );
          ]
      in

      let template =
        OpenAPIResourceTemplate.create ~client ~route ~uri_template
          ~name:template_name ~description ~parameters ~tags:(Set.to_list tags)
          ?timeout ()
      in
      ...
      Option.iter mcp_component_fn ~f:(fun f ->
          try
            f route template;
            log_debug (fun m ->
                m "Template %s customized by component_fn" template_name)
          with e ->
            log_warning (fun m ->
                m
                  "Error in component_fn for template %s: %s. Using component \
                   as-is."
    ...
    initializer
      ...
      List.iter routes ~f:(fun route ->
          ...
          let mcp_type =
            ...
            | Some f ->
              Option.value
                (f route route_map.mcp_type)
                ~default:route_map.mcp_type
          ...
          | MCPType.Tool ->
            ...
            log_debug (fun m ->
                m "Registered TOOL: %s (%s %s) with tags: %s" tool_name
                  (HttpRoute.get_method route)
                  route.path
          ...
          | MCPType.Resource ->
            ...
            log_debug (fun m ->
                m "Registered RESOURCE: %s (%s %s) with tags: %s"
                  (Uri.to_string uri)
                  (HttpRoute.get_method route)
                  route.path
                  (String.concat ~sep:", " route.tags))
          | MCPType.ResourceTemplate ->
            ...
            (* Register resource template directly with the MCP server
               framework *)
            log_debug (fun m ->
                m "Registered TEMPLATE: %s (%s %s) with tags: %s" uri_template
                  (HttpRoute.get_method route)
                  route.path
                  (String.concat ~sep:", " route.tags))
          | MCPType.Exclude ->
            log_info (fun m ->
                m "Excluding route: %s %s"
                  (HttpRoute.get_method route)
                  route.path))
  end
...
let format_description_with_responses ?(parameters = []) ?request_body
    ~responses base_description =
  ...
  if not (List.is_empty path_params) then (
    ...
    List.iter path_params ~f:(fun param ->
        ...
        Buffer.add_string buf
          (sprintf "\n- **%s**%s: %s" param.name required_marker desc)));
  ...
  Option.iter request_body ~f:(fun rb ->
      Option.iter rb.description ~f:(fun desc ->
          ...
          match
            List.Assoc.find rb.content_schema ~equal:String.equal
              "application/json"
          with
          | Some (`Assoc fields) -> (
  ...
  if not (List.is_empty responses) then (
    ...
    List.iter responses ~f:(fun (status, resp) ->
        ...
        match
          List.Assoc.find resp.content_schema ~equal:String.equal
            "application/json"
        with
        | Some schema ->
...
let handle_array_parameter param value =
  ...
  | `Assoc fields -> (
    ...
    | Some (`String "array") ->
      (* Check if explode is specified (default is true for query, false for
         path) *)
      let default_explode =
      ...
      if explode then (* Return as separate values *)
        value
      else (* Join with commas *)
  ...
  let rec clean = function
    | `Assoc fields ->
      ...
      let process_field (key, value) =
        ...
        | "additionalProperties" when value <> `Bool true -> (key, clean value)
        | _ -> (key, value)
      in
...
let validate_schema schema =
  try
    ...
    | `Assoc _ ->
      if
        String.is_prefix version ~prefix:"3.0"
        || String.is_prefix version ~prefix:"3.1"
      then
        let rec validate_schema_node = function
          | `Assoc fields -> (
            List.for_all fields ~f:(fun (_, value) ->
          ...
          | _ -> true
        in
        if validate_schema_node schema then Ok version
        else Error "Invalid schema: invalid enum values or format"
      else
        Error
          (sprintf
             "Unsupported OpenAPI version: %s. Only 3.0.x and 3.1.x are \
              supported."
             version)
...
let rec generate_example_from_schema schema =
  ...
  | `Assoc fields -> (
    ...
    | Some (`String "object") ->
      ...
      in
      `Assoc
        (List.map properties ~f:(fun (name, prop_schema) ->
...
let rec replace_ref_with_defs root_schema schema =
    ...
    | Some (`String ref) when String.is_prefix ref ~prefix:"#/" -> (
      ...
      let rec find_def schema parts =
        ...
        | part :: rest, `Assoc fields ->
          Option.bind (List.Assoc.find fields ~equal:String.equal part)
            ~f:(fun s -> find_def s rest)
        | _, _ -> None
      in
  ...
  | `List items -> `List (List.map items ~f:(replace_ref_with_defs root_schema))
  | other -> other
