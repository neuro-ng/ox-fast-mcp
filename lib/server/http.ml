...
let current_http_request : Request.t option Lwt.key = Lwt.new_key ()
...
let request_context_middleware =
  {
    ...
    handler =
      (fun next request -> with_http_request request (fun () -> next request));
  }
    ...
    [
      {
        ...
        handler =
          (fun next request ->
            let* auth_result =
              Bearer_auth_backend.authenticate
              ...
              `Assoc
                [
                  ...
                  ( "service_documentation",
                    match auth.service_documentation_url with
                    | Some url -> `String url
                    | None -> `Null );
                ]
            in
...
let create_sse_app ~server ~message_path ~sse_path ?auth ?(debug = false)
    ?routes ?middleware () =
  let message_path =
    if String.is_suffix ~suffix:"/" message_path then message_path
    else message_path ^ "/"
  in
  ...
  (match auth with
  | Some auth ->
    let auth_middleware, auth_routes, required_scopes =
            ...
            handler =
              (fun request ->
                ...
                Require_auth_middleware.handle
                  ...
                  auth_result request (handle_sse request));
          };
          {
            path = message_path;
            methods = [ "POST" ];
  ...
  | None ->
    ...
    server_routes :=
      !server_routes
      @ [
          { path = sse_path; methods = [ "GET" ]; handler = handle_sse };
          {
            path = message_path;
            methods = [ "POST" ];
...
let create_streamable_http_app ~server ~streamable_http_path ?event_store ?auth
    ?(json_response = false) ?(stateless_http = false) ?(debug = false) ?routes
    ?middleware () =
  let streamable_http_path =
    if String.is_suffix ~suffix:"/" streamable_http_path then
  ...
  let session_manager =
    Mcp.Server.Streamable_http.create_session_manager ~server ?event_store
      ~json_response ~stateless:stateless_http ()
  in

  (* Streamable HTTP handler *)
  let handle_streamable_http request =
    try%lwt Mcp.Server.Streamable_http.handle_request session_manager request
    with Runtime_error msg as e ->
      if
        String.equal msg
      ...
      then
        let new_msg =
          "FastMCP's StreamableHTTPSessionManager task group was not \
           initialized. This commonly occurs when the FastMCP application's \
           lifespan is not passed to the parent ASGI application (e.g., \
           FastAPI or Starlette). Please ensure you are setting \
           `lifespan=mcp_app.lifespan` in your parent app's constructor, where \
           `mcp_app` is the application instance returned by \
           `fastmcp_instance.http_app()`. \n\
