...
module type S = sig
  ...
  val on_call_tool : t -> context -> Results.call_tool_result call_next -> Results.call_tool_result Deferred.t
  val on_read_resource : t -> context -> Read_resource_result.t call_next -> Read_resource_result.t Deferred.t
  val on_get_prompt : t -> context -> Get_prompt_result.t call_next -> Get_prompt_result.t Deferred.t
  val on_list_tools : t -> context -> Results.list_tools_result call_next -> Results.list_tools_result Deferred.t
  val on_list_resources : t -> context -> Results.list_resources_result call_next -> Results.list_resources_result Deferred.t
  val on_list_resource_templates : t -> context -> Results.list_resource_templates_result call_next -> Results.list_resource_templates_result Deferred.t
  val on_list_prompts : t -> context -> Results.list_prompts_result call_next -> Results.list_prompts_result Deferred.t
end

let make_middleware_wrapper (type t) (module M : S with type t = t) (middleware : t) (call_next : 'a call_next) : 'a call_next =
...
let copy_context context
    ...
    () =
  {
    ...
    fastmcp_context = Option.value fastmcp_context ~default:context.fastmcp_context;
...
module Base = struct
  ...
  let dispatch_handler _t context call_next =
    ...
    | Some "resources/read" -> handler := (fun ctx -> on_read_resource _t ctx !handler)
    ...
    | Some "resources/list" -> handler := (fun ctx -> on_list_resources _t ctx !handler)
    | Some "resources/templates/list" -> handler := (fun ctx -> on_list_resource_templates _t ctx !handler)
    | Some "prompts/list" -> handler := (fun ctx -> on_list_prompts _t ctx !handler)
    | _ -> ());
