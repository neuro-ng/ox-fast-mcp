...
val make_middleware_wrapper : (module S with type t = 't) -> 't -> 'a call_next -> 'a call_next
(** Create a wrapper that applies a single middleware to a context *)

(** Helper functions *)
val copy_context : context -> ?message:Yojson.Safe.t -> ?fastmcp_context:Context.t option -> ?source:[ `Client | `Server ] -> ?type_:[ `Request | `Notification ] -> ?method_:string option -> ?timestamp:Time.t -> ?params:Yojson.Safe.t -> ?id:string option -> ?resource:string option -> unit -> context
