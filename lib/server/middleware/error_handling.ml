...
let create
    ?(logger = Logger.create "fastmcp.errors")
...
let log_error t error context =
  let error_type = Exn.to_string_hum error |> String.split ~on:' ' |> List.hd_exn in
  let method_ = Option.value context.method_ ~default:"unknown" in
  let error_key = sprintf "%s:%s" error_type method_ in
  
  (* Update error counts *)
  t.error_counts <- String.Map.update t.error_counts error_key ~f:(function
    | None -> 1
    | Some count -> count + 1);

  let base_message = sprintf "Error in %s: %s: %s" method_ error_type (Exn.to_string error) in
  
  if t.include_traceback then
    Logger.error t.logger (fun () -> sprintf "%s\n%s" base_message (Backtrace.to_string (Backtrace.get ())))
  else
    Logger.error t.logger (fun () -> base_message);

  (* Call error callback if provided *)
  match t.error_callback with
  | Some callback ->
    (try callback error context with
    | exn -> Logger.error t.logger (fun () -> sprintf "Error in error callback: %s" (Exn.to_string exn)))
  | None -> ()
...
let transform_error t error =
  ...
  | Invalid_argument msg | Arg.Bad msg ->
    Mcp_error { code = -32602; message = sprintf "Invalid params: %s" msg; details = None }
  | Not_found | Caml.Not_found | Not_found_s _ ->
    Mcp_error { code = -32001; message = sprintf "Resource not found: %s" (Exn.to_string error); details = None }
  | Unix.Unix_error(EACCES, _, _) ->
    Mcp_error { code = -32000; message = sprintf "Permission denied: %s" (Exn.to_string error); details = None }
  | Async_unix.Timeout ->
    Mcp_error { code = -32000; message = sprintf "Request timeout: %s" (Exn.to_string error); details = None }
  | exn ->
    Mcp_error { code = -32603; message = sprintf "Internal error: %s" (Exn.to_string exn); details = None }
...
module Retry = struct
  ...
  let create
      ...
      () =
    { max_retries; base_delay; max_delay; backoff_multiplier; retry_exceptions; logger }

  let should_retry t error =
    List.exists t.retry_exceptions ~f:(fun predicate -> predicate error)

  let calculate_delay t attempt =
    let delay = t.base_delay *. (t.backoff_multiplier ** Float.of_int attempt) in
      ...
      | Error error as e ->
        if attempt = t.max_retries || not (should_retry t error) then
          ...
          Logger.warning t.logger (fun () ->
              sprintf "Request %s failed (attempt %d/%d): %s: %s. Retrying in %.1fs..."
                (Option.value context.method_ ~default:"unknown")
                (attempt + 1)
                (t.max_retries + 1)
                (Exn.to_string_hum error)
                (Exn.to_string error)
                delay);
          Clock.after (Time.Span.of_sec delay)
          >>= fun () -> try_request (attempt + 1)
    in
    try_request 0
    >>= function
    | Ok result -> return result
