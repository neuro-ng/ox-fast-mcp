...
let default_config () =
  { transport = Stdio; auth = None; log_level = Info; config_file = None }
...
let parse_args config =
  ...
  let spec =
    [
      ...
      ( "--auth-bearer",
        Arg.String set_auth_bearer,
        " <token> Use bearer token authentication" );
      ( "--log-level",
        Arg.String set_log_level,
        " <level> Set log level (debug|info|notice|warning|error)" );
      ( "--config",
  ...
  | _ ->
    Printf.eprintf
      "Error: HTTP transport requires --http-host, --http-port, and --http-path\n"
...
let print_config config =
  let transport_str =
    ...
    | StreamableHttp { host; port; path } ->
      Printf.sprintf "streamable-http://%s:%d%s" host port path
  in
...
(** CLI subcommands *)
type subcommand = Server | Client | Test | Help
...
let print_help () =
  ...
  Printf.printf
    "    --http-host <host>          HTTP host (use with --http-port and \
     --http-path)\n";
  ...
  Printf.printf
    "    fastmcp client --http-host localhost --http-port 8080 --http-path /mcp\n";
