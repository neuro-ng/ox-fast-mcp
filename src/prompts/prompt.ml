...
(* TODO: Define execution_context type or import from proper module *)
type execution_context = unit
type prompt_message = json (* TODO: Use proper type from fmcp_types *)
...
(* TODO: Use proper type from fmcp_types *)
...
let create_manager ?(duplicate_behavior = `Warn) ?(mask_error_details = false)
    () =
  { prompts = Hashtbl.create 16; duplicate_behavior; mask_error_details }

(** Create a new function prompt *)
let create_prompt ~name ~description ?(arguments = None) ?(enabled = true)
    ?(tags = []) ?(annotations = None) handler =
  { name; description; arguments; handler; enabled; tags; annotations }
...
let register_prompt manager prompt =
  ...
  | true, `Replace -> Hashtbl.replace manager.prompts prompt.name prompt
  | true, `Ignore ->
    (* Do nothing - keep existing prompt *)
    ()
  | false, _ -> Hashtbl.add manager.prompts prompt.name prompt

(** Remove a prompt from the manager *)
let remove_prompt manager name = Hashtbl.remove manager.prompts name

(** Get a prompt by name *)
let get_prompt manager name =
  try Some (Hashtbl.find manager.prompts name) with Not_found -> None
...
let get_enabled_prompts manager =
  Hashtbl.fold
    (fun _name prompt acc -> if prompt.enabled then prompt :: acc else acc)
    manager.prompts []

(** Filter prompts by tags *)
let filter_prompts_by_tags manager tags =
  Hashtbl.fold
    (fun _name prompt acc ->
      let has_any_tag =
        List.exists (fun tag -> List.mem tag prompt.tags) tags
      in
...
let execute_prompt manager prompt_name context args =
  ...
  | None -> Lwt.fail (Failure ("Prompt not found: " ^ prompt_name))
  ...
  | Some prompt ->
    Lwt.catch
      ...
      (fun exn ->
        let error_msg =
          if manager.mask_error_details then "Prompt execution failed"
          else "Prompt execution failed: " ^ Printexc.to_string exn
        in
...
let add_prompt_tags manager name additional_tags =
  ...
  | Some prompt ->
    let combined_tags =
      List.rev_append additional_tags prompt.tags
...
let remove_prompt_tags manager name tags_to_remove =
  ...
  | Some prompt ->
    let filtered_tags =
      List.filter (fun tag -> not (List.mem tag tags_to_remove)) prompt.tags
    in
...
let create_summarization_prompt () =
  let handler _ctx args =
    ...
    let max_length =
      match List.assoc_opt "max_length" args with
      | Some (`Int l) -> l
      | _ -> 100
    in
      ...
      [
        {
          role = "system";
          content =
            create_text_content "You are a helpful summarization assistant.";
        };
        {
          role = "user";
          content = create_text_content ("Please summarize: " ^ text);
        };
        {
          role = "assistant";
          content = create_text_content ("Summary: " ^ summary_text);
        };
      ]
  in
  ...
  create_prompt ~name:"summarize" ~description:"Summarize text content"
    ~arguments
...
let create_translation_prompt () =
  let handler _ctx args =
    ...
    let target_lang =
      match List.assoc_opt "target_language" args with
      ...
      [
        {
          ...
          content =
            create_text_content
              ("You are a professional translator. Translate to " ^ target_lang
             ^ ".");
        };
    ...
    Some
      [
        ...
        };
        {
          ...
          description = Some "Target language";
          required = true;
        };
      ]
  in

  create_prompt ~name:"translate"
    ~description:"Translate text to target language" ~arguments
    ~tags:[ "text"; "translation" ] handler

(** Helper: Create a creative writing prompt *)
let create_creative_writing_prompt () =
  let handler _ctx args =
    let topic =
      match List.assoc_opt "topic" args with
      | Some (`String t) -> t
      | _ -> "adventure"
    in

    let style =
      match List.assoc_opt "style" args with
      | Some (`String s) -> s
      | _ -> "narrative"
    in
      ...
      [
        {
          ...
          content =
            create_text_content
              ("You are a creative writer specializing in " ^ style ^ " style.");
        };
        {
          role = "user";
          content = create_text_content ("Write about: " ^ topic);
        };
      ]
  in
  ...
  let arguments =
    Some
      [
        {
          name = "topic";
          description = Some "Topic to write about";
          required = true;
        };
  ...
  create_prompt ~name:"creative_write"
    ~description:"Generate creative writing content" ~arguments
    ~tags:[ "creative"; "writing" ] handler

(** Clear all prompts from manager *)
let clear_prompts manager = Hashtbl.clear manager.prompts
...
let get_prompt_stats manager =
  ...
  let enabled_count =
    List.length (List.filter (fun p -> p.enabled) all_prompts)
  in
  let all_tags =
    List.fold_left
      (fun acc prompt -> List.rev_append prompt.tags acc)
      [] all_prompts
    |> List.sort_uniq String.compare
  in
...
module Template = struct
    ...
    List.fold_left
      (fun acc (key, value) ->
        ...
        let value_str =
          ...
          | _ -> "null"
        in
        (* Simple string replacement - split on placeholder and rejoin with
           value *)
        let rec replace s =
  ...
  let create_template_prompt ~name ~description ~template ?(arguments = None)
      ?(tags = []) () =
    let handler _ctx args =
