(** Prompt types for OxFastMCP
    ...
    Follows the same pattern as tool_types.ml
...
(** Set prompt key *)
let with_key (prompt : t) (new_key : string) : t = Components.with_key prompt new_key
...
let render (prompt : t) ~(arguments : (string * json) list option) :
  ...
  if not prompt.enabled then
    Deferred.return
      (Error
         {
           Ox_fast_mcp.Exceptions.message = Printf.sprintf "Prompt '%s' is disabled" prompt.name;
           code = Some 403;
           data = None;
         })
  else
...
let create_function_prompt ~name ?description ?(arguments = []) ?(tags = []) ?key
    ?(enabled = true) (fn : prompt_handler) : t =
  let prompt_data = { arguments = if List.is_empty arguments then None else Some arguments } in
  ...
  Components.create
    ...
    ?key
    ~enabled
    ~data:prompt_component_data
    ()

(** {1 MCP Integration} *)

(** Convert to MCP prompt *)
let to_mcp_prompt ?(_include_fastmcp_meta = false) (prompt : t) : Mcp.Types.prompt =
  let pf = get_function prompt.data.kind in
...
let from_component (component : prompt_component_data Components.component) : t = component
