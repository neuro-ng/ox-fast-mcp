...
let list (manager : t) : Prompt_types.t list =
  Hashtbl.data manager.prompts

let list_enabled (manager : t) : Prompt_types.t list =
  Hashtbl.data manager.prompts
  |> List.filter ~f:Prompt_types.is_enabled
...
let count (manager : t) : int =
  Hashtbl.length manager.prompts
...
let find_by_name (manager : t) (name : string) : Prompt_types.t option =
  Hashtbl.data manager.prompts
  |> List.find ~f:(fun prompt -> String.equal (Prompt_types.get_name prompt) name)

(** {1 Prompt Rendering} *)

let render_prompt (manager : t) (key : string) ~(arguments : (string * Yojson.Safe.t) list option) :
    (Prompt_types.prompt_message list, Ox_fast_mcp.Exceptions.error_data) Deferred.Result.t =
  match get manager key with
  | None ->
    let error_data = {
      Ox_fast_mcp.Exceptions.message = Printf.sprintf "Prompt not found: %s" key;
      code = Some 404;
      data = None;
    } in
  ...
  | Some prompt ->
    if not (Prompt_types.is_enabled prompt) then
      let error_data = {
        Ox_fast_mcp.Exceptions.message = Printf.sprintf "Prompt is disabled: %s" key;
        code = Some 403;
        data = None;
      } in
      return (Error error_data)
      ...
      | Error exn ->
        let error_msg =
          if manager.mask_error_details then
            ...
            Printf.sprintf "Prompt rendering failed: %s" (Exn.to_string exn)
        in
        let error_data = {
...
let get_prompts (manager : t) : Prompt_types.t list =
  list manager
