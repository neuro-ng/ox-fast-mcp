...
let create_type_adapter ~name ~parameters ~return_type ?description
    validate_and_call =
  let sig_val =
    {
      name;
  ...
  let json_schema () =
    let properties =
      List.map parameters ~f:(fun param ->
          ...
          let with_description =
            match param.param_description with
          ...
          let with_default =
            ...
            | Some default ->
              ("default", json_value_to_json default) :: with_description
    ...
    `Assoc
      [
        ...
        ( "required",
          `List (List.map parameters ~f:(fun p -> `String p.param_name)) );
      ]
  in
  ...
  | None ->
    ...
    let validate_and_call args =
      ...
      let missing_required =
        List.filter func_signature.parameters ~f:(fun param ->
            param.param_required
            && not (List.Assoc.mem args ~equal:String.equal param.param_name))
      in

      if List.length missing_required > 0 then
        let missing_names =
          List.map missing_required ~f:(fun p -> p.param_name)
        in
        Result.error
          ("Missing required parameters: "
          ^ String.concat ~sep:", " missing_names)
      else
    ...
    let adapter =
      create_type_adapter ~name:func_signature.name
        ~parameters:func_signature.parameters
        ~return_type:func_signature.return_type
        ?description:func_signature.description validate_and_call
    in
...
type jsonrpc_error = { code : int; message : string; data : json option }
...
type prompt_message = { role : string; content : content_type }
(** Message for prompts *)
...
(** Type adapter functionality - OCaml equivalent of Python's Pydantic
    TypeAdapter *)
  ...
  let properties =
    List.map params ~f:(fun param ->
        ...
        let with_default =
          match param.param_default with
          | Some default ->
            ("default", (default :> Yojson.Safe.t)) :: with_description
          | None -> with_description
        in
        (param.param_name, `Assoc with_default))
  in
...
let filter_schema = function
  | `Assoc props ->
    `Assoc (List.filter props ~f:(fun (k, _) -> not (String.equal k "$schema")))
  | x -> x
...
let find_param_by_type (params : parameter list) (target_type : string) :
    parameter option =
  List.find_opt (fun param -> param.param_type = target_type) params

(** Find parameter by name in function signature *)
let find_param_by_name (params : parameter list) (target_name : string) :
    parameter option =
...
let content_type_of_yojson = function
  ...
  | `Assoc l when List.assoc_opt "type" l = Some (`String "image") -> (
    ...
    | Some (`String data), Some (`String mime_type) ->
      let annotations =
        match List.assoc_opt "annotations" l with
        | Some (`Assoc annots) -> Some annots
        | _ -> None
      in
      let format =
        match List.assoc_opt "format" l with
...
let transport_config_of_yojson = function
  ...
  | `Assoc l when List.assoc_opt "type" l = Some (`String "http") -> (
    match
      (List.assoc_opt "host" l, List.assoc_opt "port" l, List.assoc_opt "path" l)
    with
    | Some (`String host), Some (`Int port), Some (`String path) ->
  ...
  | `Assoc l when List.assoc_opt "type" l = Some (`String "streamable_http")
    ...
    with
    | Some (`String host), Some (`Int port), Some (`String path) ->
      Ok (StreamableHttp { host; port; path })
...
let auth_config_of_yojson = function
  ...
  | `Assoc l when List.assoc "type" l = `String "oauth" -> (
    ...
    | Some (`String client_id), Some (`String client_secret) ->
      Ok (OAuth { client_id; client_secret })
...
let jsonrpc_request_to_yojson (req : jsonrpc_request) =
  let base =
    [ ("jsonrpc", `String req.jsonrpc); ("method", `String req.method_) ]
  in
  let with_id =
    match req.id with
...
let resource_def_of_yojson = function
  | `Assoc l -> (
    ...
    | Some (`String uri) ->
      ...
      in
      let description =
        match List.assoc_opt "description" l with
        | Some (`String d) -> Some d
        | _ -> None
      in
      let mime_type =
        match List.assoc_opt "mimeType" l with
...
let prompt_def_to_yojson (prompt : prompt_def) =
  let base =
    [
      ("name", `String prompt.name); ("description", `String prompt.description);
    ]
  in
  let with_args =
      ...
      let arg_json =
        List.map
          (fun (arg : prompt_argument) ->
            let base =
              [ ("name", `String arg.name); ("required", `Bool arg.required) ]
            in
            let with_desc =
...
let prompt_def_of_yojson = function
  | `Assoc l -> (
    ...
    | Some (`String name), Some (`String description) ->
      let arguments =
        match List.assoc_opt "arguments" l with
        | Some (`List arg_list) ->
          let args =
            List.map
              (function
                | `Assoc arg_l -> (
                  match
                    ( List.assoc_opt "name" arg_l,
                      List.assoc_opt "required" arg_l )
                  with
                  | Some (`String arg_name), Some (`Bool required) ->
                    let arg_description =
                      match List.assoc_opt "description" arg_l with
                      | Some (`String d) -> Some d
                      | _ -> None
                    in
...
let progress_info_to_yojson (progress : progress_info) =
  ...
  in
  let with_total =
    match progress.total with
    | Some total -> ("total", `Float total) :: base
    | None -> base
  in
  let with_message =
    match progress.message with
...
let progress_info_of_yojson = function
  | `Assoc l -> (
    ...
    | Some token, Some (`Float progress) ->
      ...
      in
      let total =
        match List.assoc_opt "total" l with
        | Some (`Float t) -> Some t
        | _ -> None
      in
      let message =
...
let log_entry_of_yojson = function
  | `Assoc l -> (
    ...
    | Some (`String level), Some (`String message) ->
      let logger =
        match List.assoc_opt "logger" l with
        | Some (`String l) -> Some l
        | _ -> None
      in
      Ok { level; message; logger; data = None }
  ...
  module V1 = struct
    module Image = struct
      type t = {
        ...
        data : string option;
        format : string option;
        annotations : Types.Annotations.t option;
      }
      ...
      let to_image_content ?mime_type ?annotations t =
        let data =
        ...
        in
        Types.ImageContent.create ~type_:"image" ~data
          ~mime_type:(Option.value mime_type ~default:(get_mime_type t))
          ~annotations:(Option.value annotations ~default:t.annotations)
          ()
    end
    ...
    module Audio = struct
      ...
      let to_audio_content ?mime_type ?annotations t =
        let data =
          match (t.path, t.data) with
          | Some path, None -> In_channel.read_all path |> Base64.encode_exn
          | None, Some data -> Base64.encode_exn data
        ...
        in
        Types.AudioContent.create ~type_:"audio" ~data
          ~mime_type:(Option.value mime_type ~default:(get_mime_type t))
          ~annotations:(Option.value annotations ~default:t.annotations)
          ()
    end
    ...
    module File = struct
      ...
      let get_mime_type t =
        ...
        | Some fmt ->
          ...
          if List.mem [ "plain"; "txt"; "text" ] fmt ~equal:String.equal then
            "text/plain"
      ...
      let to_resource_content ?mime_type ?annotations t =
        let mime_type = Option.value mime_type ~default:(get_mime_type t) in
        let uri, raw_data =
          match (t.path, t.data) with
          | Some path, None ->
            let path = Filename.realpath path in
            let uri = Uri.of_string (sprintf "file://%s" path) in
            (uri, In_channel.read_all path)
          | None, Some data ->
            let ext = String.split mime_type ~on:'/' |> List.last_exn in
            let name = Option.value t.name ~default:"resource" in
            let uri = Uri.of_string (sprintf "file:///%s.%s" name ext) in
            (uri, data)
          | _ -> failwith "No resource data available"
        in
        let resource =
          if String.is_prefix mime_type ~prefix:"text/" then
            let text =
              try String.of_bytes raw_data
              with _ -> String.of_bytes_exn raw_data ~strip_cr:true
            in
            Types.TextResourceContents.create ~text ~mime_type ~uri ()
            |> fun x -> `Text x
          else
            let blob = Base64.encode_exn raw_data in
            Types.BlobResourceContents.create ~blob ~mime_type ~uri ()
            |> fun x -> `Blob x
        in
        Types.EmbeddedResource.create ~type_:"resource" ~resource
          ~annotations:(Option.value annotations ~default:t.annotations)
          ()
    end
  end
