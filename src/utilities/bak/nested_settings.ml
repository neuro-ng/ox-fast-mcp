...
module Field_value = struct
  type t =
    | String of string [@to_yojson fun s -> `String s]
        [@of_yojson
          fun j ->
            match j with
            | `String s -> Ok s
            | _ -> Error "Expected string"]
    | Int of int [@to_yojson fun i -> `Int i]
        [@of_yojson
          fun j ->
            match j with
            | `Int i -> Ok i
            | _ -> Error "Expected int"]
    | Float of float [@to_yojson fun f -> `Float f]
        [@of_yojson
          fun j ->
            match j with
            | `Float f -> Ok f
            | _ -> Error "Expected float"]
    | Bool of bool [@to_yojson fun b -> `Bool b]
        [@of_yojson
          fun j ->
            match j with
            | `Bool b -> Ok b
            | _ -> Error "Expected bool"]
    | List of t list [@to_yojson fun l -> `List (List.map l ~f:yojson_of_t)]
        [@of_yojson
          fun j ->
            match j with
            | `List l -> List.map_result l ~f:t_of_yojson
            | _ -> Error "Expected list"]
    | Dict of (string * t) list
        [@to_yojson
          fun d -> `Assoc (List.map d ~f:(fun (k, v) -> (k, yojson_of_t v)))]
        [@of_yojson
          fun j ->
            match j with
            | `Assoc d ->
              List.map_result d ~f:(fun (k, v) ->
                  t_of_yojson v |> Result.map ~f:(fun v -> (k, v)))
            | _ -> Error "Expected dict"]
    | Null [@to_yojson fun _ -> `Null]
        [@of_yojson
          fun j ->
            match j with
            | `Null -> Ok Null
  ...
  let set_value t name value =
    {
      t with
      values =
        (name, value) :: List.filter t.values ~f:(fun (n, _) -> n <> name);
    }

  let get_value t name = List.Assoc.find t.values name ~equal:String.equal
  ...
  let validate t =
    let validate_required_fields () =
      List.fold_result t.fields ~init:() ~f:(fun () (name, metadata) ->
          if
            metadata.required
            && not (List.Assoc.mem t.values name ~equal:String.equal)
...
let validate_max max s =
  try
    let n = Float.of_string s in
    if Float.(n <= max) then Ok () else Error (sprintf "Must be at most %f" max)
  with _ -> Error "Not a number"

let validate_enum values s =
  if List.mem values s ~equal:String.equal then Ok ()
  else Error (sprintf "Must be one of: %s" (String.concat ~sep:", " values))
...
let rec merge_value v1 v2 =
  match (v1, v2) with
  | Dict d1, Dict d2 -> Dict (merge_dicts d1 d2)
