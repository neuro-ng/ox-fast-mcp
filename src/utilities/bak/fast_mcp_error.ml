...
let create_error ?(details = None) code message = { code; message; details }
...
let error_data_of_yojson = function
  | `Assoc fields -> (
    match
      ( List.Assoc.find fields ~equal:String.equal "code",
        List.Assoc.find fields ~equal:String.equal "message",
        List.Assoc.find fields ~equal:String.equal "details" )
    with
      ...
      Ok
        {
          ...
          message;
          details =
            Option.filter details ~f:(fun x -> not (Yojson.Safe.equal x `Null));
        }
