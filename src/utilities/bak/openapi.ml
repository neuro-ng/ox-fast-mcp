...
module Parameter = struct
  ...
  let get_location (param : parameter_info) =
    string_of_parameter_location param.location
...
let parameter_info_of_json json =
  ...
  in
  let location =
    Yojson.Safe.Util.member "in" json
    |> Yojson.Safe.Util.to_string |> parameter_location_of_string
  in
  let required =
    Yojson.Safe.Util.member "required" json
  ...
  in
  let description =
    Yojson.Safe.Util.member "description" json
    |> Yojson.Safe.Util.to_string_option
  in
  match location with
  | Some location -> Some { name; location; required; schema; description }
  | None -> None

let rec deep_copy_json = function
  | `Assoc fields ->
    `Assoc (List.map (fun (k, v) -> (k, deep_copy_json v)) fields)
  ...
  | `Null -> `Null
  | `Tuple items ->
    `List (List.map deep_copy_json items) (* Convert tuple to list *)
  | `Variant (s, opt) -> (
    (* Convert variant to object *)
    match opt with
...
let request_body_info_of_json json =
  ...
  in
  let content =
    Yojson.Safe.Util.member "content" json |> Yojson.Safe.Util.to_assoc
  in
  let content_schema =
    List.map
      (fun (media_type, schema) ->
  ...
  let description =
    ...
    |> Yojson.Safe.Util.to_string_option
  in
  Some { required; content_schema; description }
...
let response_info_of_json json =
  ...
  let content_schema =
    List.map
      ...
      content
  in
  Some { description; content_schema }
  ...
  `Assoc
    [
      ...
      ( "operation_id",
        ...
        | None -> `Null );
      ( "summary",
        match route.summary with
        | Some s -> `String s
        | None -> `Null );
      ( "description",
        match route.description with
        | Some d -> `String d
        | None -> `Null );
      ...
      ( "request_body",
        ...
        | Some rb -> json_of_request_body_info rb
        | None -> `Null );
      ( "responses",
        `Assoc
          (List.map
             (fun (code, info) -> (code, json_of_response_info info))
             route.responses) );
...
let http_route_of_json json =
  ...
  try
    ...
    let parameters =
      member "parameters" json |> to_list
      |> List.filter_map parameter_info_of_json
    in
    let request_body =
      member "request_body" json |> request_body_info_of_json
    in
    let responses =
      member "responses" json |> to_assoc
...
let rec resolve_ref schema ref_path =
  let parts =
    String.split_on_char '/'
      (String.sub ref_path 2 (String.length ref_path - 2))
  in
    ...
    | part :: rest -> (
      ...
      | `Assoc fields -> (
        ...
        | None ->
          raise
            (Invalid_argument
               (Printf.sprintf "Reference part '%s' not found" part)))
      | _ ->
        raise
          (Invalid_argument (Printf.sprintf "Cannot traverse part '%s'" part)))
  in
  try
...
let rec replace_ref_with_defs schema json =
    ...
    let process_field (key, value) =
      ...
      | "$ref", `String ref ->
        if String.starts_with ~prefix:"#/components/schemas/" ref then
          ...
          if resolved = `Null then (key, value)
          else (key, replace_ref_with_defs schema resolved)
        else
          let error_msg =
            Printf.sprintf
              "External or non-local reference not supported: %s. FastMCP only \
               supports local schema references (starting with #/)."
              ref
          in
  ...
  | `List items -> `List (List.map (replace_ref_with_defs schema) items)
  | other -> other
...
let request_body_info_of_json_with_schema schema_context json =
  ...
  let content_schema =
    List.map
      (fun (media_type, schema) ->
        let raw_schema =
          deep_copy_json (Yojson.Safe.Util.member "schema" schema)
        in
...
let combine_schemas route =
  ...
  let add_schema schema combined =
    ...
    | `Assoc _, `Assoc combined_fields ->
      let properties =
        Yojson.Safe.Util.member "properties" schema |> Yojson.Safe.Util.to_assoc
      in
      let required =
        Yojson.Safe.Util.member "required" schema |> Yojson.Safe.Util.to_list
      in
      let new_properties =
        match List.assoc_opt "properties" combined_fields with
        | Some (`Assoc existing) -> `Assoc (existing @ properties)
        | _ -> `Assoc properties
      in
      let new_required =
        match List.assoc_opt "required" combined_fields with
      ...
      `Assoc
        ...
        :: List.filter
             (fun (k, _) -> k <> "properties" && k <> "required")
             combined_fields)
    | _ -> combined
  in
  let combined_with_params =
    List.fold_left
      (fun acc param -> add_schema param.schema acc)
      combined route.parameters
  in
  let combined_with_body =
    match route.request_body with
    | Some body ->
      List.fold_left
        (fun acc (_, schema) -> add_schema schema acc)
        combined_with_params body.content_schema
    | None -> combined_with_params
  in
  let combined_with_responses =
    List.fold_left
      (fun acc (_, resp) ->
        List.fold_left
          (fun acc (_, schema) -> add_schema schema acc)
          acc resp.content_schema)
      combined_with_body route.responses
  in
  let combined_with_defs =
    `Assoc
      (List.filter
         (fun (k, _) -> k <> "$defs")
         (Yojson.Safe.Util.to_assoc combined_with_responses)
      @ [ ("$defs", `Assoc route.schema_definitions) ])
  in
  let with_refs = replace_ref_with_defs combined_with_defs combined_with_defs in
  Json_schema.compress_schema ~prune_defs:true ~prune_additional_properties:true
    ~prune_titles:true with_refs
...
let validate_schema schema =
  try
    let version =
      Yojson.Safe.Util.member "openapi" schema |> Yojson.Safe.Util.to_string
    in
    let paths = Yojson.Safe.Util.member "paths" schema in
    match paths with
    | `Assoc _ ->
      if
        String.starts_with ~prefix:"3.0" version
        || String.starts_with ~prefix:"3.1" version
      then
...
let parse_openapi_to_http_routes schema =
  ...
  | Ok version -> (
    ...
    let paths =
      Yojson.Safe.Util.member "paths" schema |> Yojson.Safe.Util.to_assoc
    in
    ...
    let schema_definitions =
      ...
      | `Null -> []
      | _ -> (
        match Yojson.Safe.Util.member "schemas" components with
        | `Assoc defs -> defs
        | _ -> [])
    in
    let extract_operation path_str path_item (method_str, operation) =
      let http_method =
        http_method_of_string (String.uppercase_ascii method_str)
      in
      match http_method with
      | None ->
      ...
      | Some http_method ->
        let operation_id =
          Yojson.Safe.Util.member "operationId" operation
          |> Yojson.Safe.Util.to_string_option
        in
        let summary =
          Yojson.Safe.Util.member "summary" operation
          |> Yojson.Safe.Util.to_string_option
        in
        let description =
          Yojson.Safe.Util.member "description" operation
          |> Yojson.Safe.Util.to_string_option
        in
        let tags =
          match Yojson.Safe.Util.member "tags" operation with
        ...
        let parameters =
          let path_params =
            match Yojson.Safe.Util.member "parameters" operation with
            | `List params -> params
            | _ -> []
          in
          let common_params =
            match Yojson.Safe.Util.member "parameters" path_item with
            | `List params -> params
            | _ -> []
          in
            ...
            | `String ref_path ->
              if String.starts_with ~prefix:"#/" ref_path then
                ...
                if resolved = `Null then (
                  Printf.eprintf
                    "Warning: Skipping broken parameter reference: %s\n"
                    ref_path;
                  `Null)
                else resolved
              else param
        ...
        let request_body =
          ...
          | body ->
            let resolved_body =
              match Yojson.Safe.Util.member "$ref" body with
              | `String ref_path ->
                if String.starts_with ~prefix:"#/" ref_path then
        ...
        in
        let responses =
          Yojson.Safe.Util.member "responses" operation
          ...
          |> List.filter_map (fun (code, resp) ->
                 ...
                 | None -> None)
        in
        let rewritten_schema_definitions =
          List.map
            (fun (name, def_schema) ->
    ...
    let extract_path_operations (path_str, path_item) =
      let operations =
        Yojson.Safe.Util.to_assoc path_item
        |> List.filter (fun (k, _) ->
               http_method_of_string (String.uppercase_ascii k) <> None)
      in
    ...
    | Invalid_argument _ as e ->
      raise e (* Re-raise Invalid_argument exceptions *)
    | e ->
...
let rec clean_schema_for_display schema =
  ...
  | `Assoc fields ->
    ...
    in
    let cleaned_fields =
      List.filter (fun (k, _) -> not (List.mem k fields_to_remove)) fields
    in
    let process_field (key, value) =
      match key with
          ...
          | None -> (
            ...
            | Some (`String "object") -> (
              ...
              | Some (`Assoc props) ->
                ...
                `Assoc
                  (List.filter
                     (fun (name, _) -> List.mem name required)
                     example_props)
...
let format_description_with_responses ?(parameters = []) ?request_body
  ...
  | Some (rb : request_body_info) when rb.description <> None -> (
    ...
    Buffer.add_string buf
      (Printf.sprintf "\n%s%s"
         (Option.value ~default:"" rb.description)
         required_marker);
  ...
  if responses <> [] then (
    ...
    let success_status =
      List.find_opt (fun (code, _) -> List.mem code success_codes) responses
    in

    List.iter
      (fun (status_code, (resp_info : response_info)) ->
        let status_marker =
          match success_status with
          | Some (code, _) when code = status_code -> " (Success)"
          | _ -> ""
        in
        let desc =
          match resp_info.description with
          | Some d -> d
          | None -> "No description."
        in
        Buffer.add_string buf
          (Printf.sprintf "\n- **%s**%s: %s" status_code status_marker desc);
        ...
        | Some schema -> (
          ...
          | `Assoc fields ->
            (match
               (List.assoc_opt "type" fields, List.assoc_opt "properties" fields)
             with
