...
let get_fn_name (_fn : 'a -> 'b) : string =
  (* OCaml doesn't have runtime function introspection like Python.
     This is a placeholder that returns a generic string.
     In practice, callers should pass function names explicitly. *)
  "<function>"
;;
  ...
  let expand_path (path_str : string) : string =
    ...
    | true ->
      (match Sys.getenv "HOME" with
       | Some home ->
         home ^ String.sub expanded_env ~pos:1 ~len:(String.length expanded_env - 1)
       | None -> expanded_env)
    | false -> expanded_env
  ;;
...
(* MIME type detection is delegated to Fmcp_types modules.
   Use Fmcp_types.Image.get_mime_type, Fmcp_types.Audio.get_mime_type, etc. *)
...
module Image = struct
  ...
  let create_from_path ?format ?annotations ~path () : (t, Error.t) Result.t Deferred.t
    =
    ignore annotations;
    ...
    let%bind data_result =
      ...
      | Some path, None ->
        ...
        (match result with
         ...
         | Error exn ->
           return (Error (Error.of_string (sprintf "Failed to read image: %s" (Exn.to_string exn)))))
      | None, Some data -> return (Ok (Base64.encode_exn data))
      | Some _, Some _ -> return (Error (Error.of_string "Both path and data provided"))
    ...
    let%bind data_result =
      ...
      | Some path, None ->
        ...
        (match result with
         | Ok contents -> return (Ok (Base64.encode_exn contents))
         | Error exn ->
           return (Error (Error.of_string (sprintf "Failed to read audio: %s" (Exn.to_string exn)))))
      | None, Some data -> return (Ok (Base64.encode_exn data))
      | Some _, Some _ -> return (Error (Error.of_string "Both path and data provided"))
        ...
        let%bind text_result =
          ...
          | None, Some path ->
            ...
            (match result with
             ...
             | Error exn ->
               return (Error (Error.of_string (sprintf "Failed to read file: %s" (Exn.to_string exn)))))
          | None, None -> return (Error (Error.of_string "No resource data available"))
        in
        (match text_result with
         | Ok text ->
           return (Ok (`Text { Mcp.Types.text
                             ; resource_contents = { uri; mime_type = Some mime_type_val; meta = None }
                             }))
         | Error e -> return (Error e))
      else
        (* Blob resource *)
