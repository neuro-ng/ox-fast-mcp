...
let yojson_of_token_response r =
  ...
  let fields = match r.refresh_token with 
    | Some rt -> ("refresh_token", `String rt) :: fields 
    | None -> fields
  in
  let fields = match r.scope with
...
let yojson_of_error_response e =
  ...
  let fields = match e.error_description with
...
let handle_token_request t ~body =
  ...
  let get_param name = 
    List.Assoc.find params name ~equal:String.equal 
  ...
  | Some "authorization_code" ->
    ...
    | Some c when List.Assoc.mem t.valid_codes c ~equal:String.equal ->
      ...
      let access_token = sprintf "mock_access_%s" (Mcp_client_auth.Crypto_utils.generate_state_token ()) in
      let refresh_token = sprintf "mock_refresh_%s" (Mcp_client_auth.Crypto_utils.generate_state_token ()) in
    ...
    | Some _ ->
      let error = { error = "invalid_grant"; error_description = Some "Invalid authorization code" } in
      `Bad_request, yojson_of_error_response error
    | None ->
      let error = { error = "invalid_request"; error_description = Some "Missing code parameter" } in
      `Bad_request, yojson_of_error_response error)
  | Some "refresh_token" ->
    (* Issue new token for refresh *)
    let access_token = sprintf "mock_refreshed_%s" (Mcp_client_auth.Crypto_utils.generate_state_token ()) in
    t.issued_tokens <- access_token :: t.issued_tokens;
    let response = {
      access_token;
      token_type = "Bearer";
    ...
    `OK, yojson_of_token_response response
  | Some gt ->
    let error = { error = "unsupported_grant_type"; error_description = Some (sprintf "Unsupported: %s" gt) } in
    `Bad_request, yojson_of_error_response error
  | None ->
    let error = { error = "invalid_request"; error_description = Some "Missing grant_type" } in
    `Bad_request, yojson_of_error_response error

let handle_registration t =
  (* Dynamic client registration - return mock client info *)
  ignore t;
  let client_id = sprintf "mock_client_%s" (String.prefix (Mcp_client_auth.Crypto_utils.generate_state_token ()) 8) in
  let client_secret = sprintf "mock_secret_%s" (Mcp_client_auth.Crypto_utils.generate_state_token ()) in
  let response = `Assoc [
...
let start t =
  ...
  let handler ~body _sock request =
    ...
    let status, response_body = match (meth, path) with
      | `GET, "/.well-known/oauth-authorization-server" ->
        `OK, oauth_metadata ~issuer
      | `GET, "/.well-known/openid-configuration" ->
        `OK, oauth_metadata ~issuer
      | `POST, "/token" ->
        handle_token_request t ~body:body_str
      | `POST, "/register" ->
        handle_registration t
      | `GET, "/authorize" ->
        ...
        let info = `Assoc [
          ("message", `String "Mock auth endpoint - use add_valid_code to simulate user auth");
          ("redirect_uri", `String (Option.value redirect_uri ~default:"none"));
          ("state", `String (Option.value state ~default:"none"));
        ] in
        `OK, info
      | _ ->
        `Not_found, `Assoc [("error", `String "Not found")]
    in
  ...
  in
  t.running <- true;
  Logs.info (fun m -> m "Mock OAuth server started on http://localhost:%d" t.port);
  return ()

let stop t =
  (* Note: In a full implementation, we'd track the server and close it.
     For tests, server runs until test process exits. *)
  t.running <- false;
  Logs.info (fun m -> m "Mock OAuth server marked as stopped");
  return ()

let is_token_valid t ~token =
  List.mem t.issued_tokens token ~equal:String.equal

let get_issuer t =
  sprintf "http://localhost:%d" t.port
