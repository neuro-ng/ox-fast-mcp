...
let sexp_of_json (j : json) : Sexp.t = Yojson.Safe.to_string j |> Sexp.of_string
...
let image_content_to_content_type img = Image img

let content_type_to_image_content = function
  | Image img -> Some img
  | _ -> None

let audio_content_to_content_type audio = Audio audio

let content_type_to_audio_content = function
  | Audio audio -> Some audio
  | _ -> None

let resource_content_to_content_type res = Resource res
...
let embedded_resource_to_content_type res =
  ...
  | Text { text; mime_type; _ } ->
    resource_content_to_content_type
      {
        data = text;
        mime_type;
        name = None;
      ...
      let to_image_content ?mime_type ?(annotations = []) t :
        ...
        let%bind data =
          ...
          | Some path, None -> (
            ...
            | Error exn ->
              failwith
                (sprintf "Failed to read image file: %s" (Exn.to_string exn)))
      ...
      let to_audio_content ?mime_type ?(annotations = []) t :
        ...
        let%bind data =
          ...
          | Some path, None -> (
            ...
            | Error exn ->
              failwith
                (sprintf "Failed to read audio file: %s" (Exn.to_string exn)))
          | None, Some data -> return (Base64.encode_exn data)
    ...
    module File = struct
      ...
      let get_mime_type t =
        ...
        | Some fmt ->
          ...
          if
            String.is_prefix ~prefix:"plain" fmt
            || String.is_prefix ~prefix:"txt" fmt
            || String.is_prefix ~prefix:"text" fmt
          then "text/plain"
      ...
      let to_resource_content ?mime_type ?(annotations = []) t :
        ...
        let ext =
          Core.Option.value_map t.format ~default:"" ~f:(fun f -> "." ^ f)
        in
          ...
          if String.is_prefix ~prefix:"text/" mime_type then
            let%bind text =
              ...
              | _, Some path -> (
                ...
                | Error exn ->
                  failwith
                    (sprintf "Failed to read text file: %s" (Exn.to_string exn))
                )
            ...
            let%bind blob_data =
              ...
              | _, Some path -> (
                ...
                | Error exn ->
                  failwith
                    (sprintf "Failed to read blob file: %s" (Exn.to_string exn))
                )
              | _ -> failwith "No resource data available"
            in
...
let rec is_class_member_of_type type_to_check base_type =
  ...
  | Some t -> (
    try
      match t with
      | `Union types ->
        List.exists types ~f:(fun t ->
            is_class_member_of_type (Some t) base_type)
      ...
      | `Class c -> (
        ...
        match (c, base) with
        | Some c', Some b' ->
          Obj.is_int (Obj.repr c') && Obj.is_int (Obj.repr b') && c' <= b'
        | _ -> false)
      | _ -> false
    with _ -> false)
  ...
  | Some s, Some p -> (
    try
      ...
      | `Class s', `Class p' -> (
        ...
        match (s', p') with
        | Some s'', Some p'' ->
          Obj.is_int (Obj.repr s'') && Obj.is_int (Obj.repr p'') && s'' <= p''
        | _ -> false)
      | _ -> false
    with _ -> false)
...
let find_kwarg_by_type func target_type =
  try
    ...
    let param_names =
      List.filter_map params ~f:(fun (name, type_) ->
          if is_class_member_of_type (Some type_) target_type then Some name
          else None)
    in
    List.hd param_names
  with _ -> None
