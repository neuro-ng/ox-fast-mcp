(** HeadlessOAuth - OAuth Client for Automated Testing
    ...
    Provides OAuth client that bypasses browser interaction for testing. Returns
    mock authorization codes directly. *)
...
let create ~server_url ~mock_auth_code ?(mock_state = "test_state_12345") () =
  ...
  let storage =
    Mcp_client_auth.Oauth2.Storage
      ( (module Test_storage : Mcp_client_auth.Oauth2.Token_storage
          with type t = Test_storage.t),
        storage_instance )
  in
...
let create_oauth_client t ~client_metadata =
  Mcp_client_auth.Oauth2.create ~server_url:t.server_url ~client_metadata
    ~storage:t.storage
  ...
  Mcp_shared.Auth.
    {
      ...
      response_types = [ `Code ];
      scope =
        (if List.is_empty scopes then None
         else Some (String.concat ~sep:" " scopes));
