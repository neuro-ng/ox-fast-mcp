...
(** Coerce a single value to the expected type
    ...
    @param expected_type
      The expected JSON type ("integer", "number", "string", "boolean")
    @param value The value to coerce
    @return Ok (coerced value) or Error (error message) *)
let coerce_value ~expected_type ~(value : Yojson.Safe.t) :
  ...
  | "integer", `String s -> (
    ...
    | i -> Ok (`Int i)
    | exception _ -> Error (sprintf "Cannot convert string '%s' to integer" s))
  (* String to number/float *)
  | "number", `String s -> (
    match Float.of_string s with
    | f -> Ok (`Float f)
    | exception _ -> Error (sprintf "Cannot convert string '%s' to number" s))
  (* String to boolean *)
  | "boolean", `String s -> (
    ...
    | _ ->
      Error
        (sprintf "Cannot convert string '%s' to boolean (use 'true' or 'false')"
           s))
  (* Type mismatch *)
  | expected, actual ->
    let actual_type =
      ...
      | `Variant _ -> "variant"
    in
    Error (sprintf "Type mismatch: expected %s, got %s" expected actual_type)
...
let get_schema_properties (schema : Yojson.Safe.t) :
...
and validate_object_lenient ~field_path ~schema
    ~(obj : (string * Yojson.Safe.t) list) :
  ...
  if not (List.is_empty missing_required) then
    ...
    let results =
      List.map obj ~f:(fun (key, value) ->
          ...
          | Some prop_schema -> (
            let new_path =
              if String.is_empty field_path then key else field_path ^ "." ^ key
            in
            match
              validate_value_lenient ~field_path:new_path ~schema:prop_schema
                ~value
            with
            | Ok v -> Ok (key, v)
            | Error e -> Error e)
    ...
    let errors =
      List.filter_map results ~f:(fun r ->
          match r with
          | Error e -> Some e
          | Ok _ -> None)
    in
  ...
  | Some item_sch ->
    ...
    let results =
      List.mapi items ~f:(fun i item ->
          ...
          validate_value_lenient ~field_path:item_path ~schema:item_sch
            ~value:item)
    in
    let errors =
      List.filter_map results ~f:(fun r ->
...
let validate_tool_input ~mode ~(schema : Yojson.Safe.t) ~(input : Yojson.Safe.t)
  ...
  | Lenient -> (
    ...
    | `List items -> (
      ...
      match get_schema_type schema with
      | Some "array" -> validate_list_lenient ~field_path:"" ~schema ~items
      | _ ->
...
let get_error_hint (err : coercion_error) : string option =
  ...
  | "integer" -> (
    ...
    | `String _ -> Some "Hint: Provide a valid integer like 42, not a string"
    | _ -> None)
  | "boolean" -> (
    match err.actual_value with
    | `String s
      when String.equal (String.lowercase s) "yes"
           || String.equal (String.lowercase s) "no" ->
      Some "Hint: Use 'true' or 'false' instead of 'yes'/'no'"
    | `Int 0 | `Int 1 -> Some "Hint: Use true or false instead of 0/1"
    | _ -> None)
  | "enum" -> Some "Hint: Check the allowed values in the error message above"
  | "required" -> Some "Hint: This field must be provided in the request"
  | _ -> None

(** Format a single error with enhanced details *)
let format_single_error (err : coercion_error) : string =
  let field_display =
    if String.is_empty err.field then "input" else sprintf "'%s'" err.field
  in
...
let format_errors errors =
  let header =
    if List.length errors = 1 then "Validation error:"
    else sprintf "Found %d validation errors:" (List.length errors)
  in
  let formatted_errors =
    errors |> List.map ~f:format_single_error |> String.concat ~sep:"\n"
  in
