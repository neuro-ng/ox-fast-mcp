...
(** Deprecated RouteType module for backward compatibility. Use MCPType instead. *)
...
module OpenAPIResourceTemplate : sig
  ...
  val create_resource :
    t -> uri:string -> params:Yojson.Safe.t -> OpenAPIResource.t Lwt.t
end
...
val handle_array_parameter :
  Utilities.Openapi.parameter_info -> string list -> string list
...
val validate_parameter_value :
  Utilities.Openapi.parameter_info -> string -> (string, string) result
(** Validate a parameter value against its schema *)

(** {2 Route Mapping} *)

val determine_route_type :
  Utilities.Openapi.http_route -> route_map list -> route_map
(** Determine the FastMCP component type for a route based on mappings *)

val generate_default_name :
  Utilities.Openapi.http_route -> string String.Map.t -> string
...
(** Generate example values from a JSON schema. Handles common types and
    formats:
...
(** Replace $ref references in a schema with their definitions. Handles both
    internal (#/...) and external references. External references are replaced
