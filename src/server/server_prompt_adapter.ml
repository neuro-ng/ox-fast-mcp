...
let create ~get_prompts ~set_prompt ~remove_prompt ~get_key ~get_name ~get_tags
  ...
  { get_prompts; set_prompt; remove_prompt; get_key; get_name; get_tags; 
    get_description; render_handler; on_duplicate }

(** {1 Manager-like Interface} *)

let has_prompt t key =
  Hashtbl.mem (t.get_prompts ()) key

let get_prompt t key =
  Hashtbl.find (t.get_prompts ()) key

let list_prompts t =
  Hashtbl.data (t.get_prompts ())

let count t =
  Hashtbl.length (t.get_prompts ())
...
let remove_prompt t key =
  if Hashtbl.mem (t.get_prompts ()) key then begin
    t.remove_prompt key;
    Ok ()
  end else
    Error (sprintf "Prompt not found: %s" key)
...
let render_prompt t key arguments =
  ...
  | None -> 
    return (Error (sprintf "Prompt not found: %s" key))
  | Some prompt ->
    Monitor.try_with (fun () -> t.render_handler prompt arguments)
    >>| function
    | Ok result -> Ok result
    | Error exn ->
