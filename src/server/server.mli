...
(** Cache management *)
module Cache : sig
  type item = { value : Yojson.Safe.t; expires_at : float option }
...
val env_bearer_auth_provider :
  ?env_var:string -> ?description:string -> unit -> auth_provider
...
val create_notification :
  string -> ?params:Yojson.Safe.t -> unit -> notification
...
type mounted_component = {
  ...
  component :
    [ `Tool of tool_info | `Resource of resource_info | `Prompt of prompt_info ];
  enabled : bool;
}
...
val logging_middleware :
  ?log_level:[ `Debug | `Info | `Warning | `Error ] -> middleware_fn
...
val remove_resource_prefix :
  string -> string -> resource_prefix_format -> string
...
module Proxy : sig
  ...
  type client = { transport : transport; mutable closed : bool }
...
module Component : sig
  ...
  val matches_tags :
    'a t -> include_tags:tag list option -> exclude_tags:tag list option -> bool
end
...
module OpenAPI : sig
  ...
  val create_response :
    code:int -> description:string -> content:(string * schema) list -> response
...
module Auth : sig
  ...
  val authenticate :
    t -> ?authenticator:string -> credentials -> session option Lwt.t
...
module Session : sig
  ...
  type config = { timeout : float; cleanup_interval : float }
  type t = { sessions : (string, data) Hashtbl.t; config : config }
...
(** Plugin system *)
module Plugin : sig
  type hook = [ `Init | `Start | `Stop | `Request | `Response | `Error ]
...
(** Metrics collection *)
module Metrics : sig
  type counter = { name : string; description : string option; value : int ref }
  type gauge = { name : string; description : string option; value : float ref }
  ...
  val create_counter :
    t -> name:string -> ?description:string -> unit -> counter
...
module Log : sig
  ...
  val create :
    ?default_level:level -> ?default_handlers:handler list -> unit -> t
...
module Config : sig
  type validation_error = { path : string list; message : string }
  ...
  type config_source = Environment | File of string | Memory
  type t
...
module Task : sig
  ...
  type progress = { current : int; total : int option; message : string option }
  ...
  val create_task :
    t -> name:string -> ?metadata:(string * string) list -> unit -> task
...
module Cache : sig
  ...
  val set :
    ?expires:float -> ?tags:string list -> t -> string -> Yojson.Safe.t -> unit
...
module RateLimit : sig
  ...
  type limit = { max_requests : int; window : window }
  type t

  val create : ?cleanup_interval:float -> unit -> t
  val add_limit : t -> key:string -> max_requests:int -> window:window -> unit
...
module Middleware : sig
  ...
  val create :
    ?handlers:handler list ->
    ?error_handlers:(exn -> unit Lwt.t) list ->
    unit ->
    t
...
module Lifecycle : sig
  type phase = [ `Initializing | `Starting | `Running | `Stopping | `Stopped ]
...
val import_server_v2 :
  'a server -> 'b server -> ?prefix:string -> unit -> unit Lwt.t

val handle_list_tools_v2 : 'a server -> tool_info list Lwt.t
(** Protocol handlers *)

val handle_call_tool_v2 :
  'a server -> string -> Yojson.Safe.t -> content list Lwt.t
...
val handle_get_prompt_v2 :
  'a server -> string -> (string * string) list -> prompt_result Lwt.t
...
val as_proxy_v2 :
  [ `Client of Proxy.client | `URL of string | `Config of Yojson.Safe.t ] ->
  'a server
(** Proxy and integration support *)

val from_openapi_v2 :
  Yojson.Safe.t ->
  ?route_maps:FastAPI.route_map list ->
  ?route_map_fn:FastAPI.route_map_fn ->
  ?auth:auth_provider ->
  unit ->
  'a server Lwt.t
...
module Proxy : sig
  ...
  val proxy_request :
    'a server -> client -> Yojson.Safe.t -> Yojson.Safe.t Lwt.t
end
