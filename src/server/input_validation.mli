...
(** Coerce a single value to the expected type
    ...
    @param expected_type Expected JSON type ("integer", "number", "string", "boolean")
    @param value Value to coerce
    @return Ok (coerced value) or Error (error message)
*)
val coerce_value :
  expected_type:string -> value:Yojson.Safe.t -> (Yojson.Safe.t, string) Result.t

(** Validate and potentially coerce tool input against a JSON schema
    
    In Lenient mode, attempts to coerce compatible types.
    In Strict mode, requires exact type matches.
