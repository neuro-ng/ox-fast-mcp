...
module Bearer_settings : sig
  type t = {
    public_key : string option;
        (** RSA public key in PEM format (for static key) *)
    ...
    required_scopes : string list option;
        (** List of required scopes for access *)
  }
...
module Env_bearer_auth_provider : sig
  ...
  (** Create a new Environment Bearer Auth Provider. Settings are loaded from
      environment variables with prefix FASTMCP_AUTH_BEARER_. Provided
