...
module Bearer_settings = struct
  ...
  let get_env_opt name : string option =
    try Some (Stdlib.Sys.getenv (env_prefix ^ name)) with _ -> None
...
module Env_bearer_auth_provider = struct
  ...
  (* The interface is expecting these to be types, but they should come from the
     provider interface *)
  type authorization_code_t = authorization_code
  type refresh_token_t = refresh_token
  type access_token_t = access_token

  (* Dummy implementations to satisfy interface - these should not be used
     directly *)
