(** Google authentication provider for OxFastMCP.
    ...
    {[
      let auth =
        Google.Google_provider.create
          ~client_id:"123456789.apps.googleusercontent.com"
          ~client_secret:"GOCSPX-abc123..." ~base_url:"https://my-server.com" ()
      in
...
let logger =
  Logging.Logger.get_logger "ox-fast-mcp.server.auth.providers.google"

(** Google OAuth endpoints *)
let google_authorization_endpoint =
  "https://accounts.google.com/o/oauth2/v2/auth"

let google_token_endpoint = "https://oauth2.googleapis.com/token"
let google_tokeninfo_api = "https://www.googleapis.com/oauth2/v1/tokeninfo"
let google_userinfo_api = "https://www.googleapis.com/oauth2/v2/userinfo"
...
module Settings = struct
  ...
  let load_from_env () =
    ...
    let client_id = get "OXFASTMCP_SERVER_AUTH_GOOGLE_CLIENT_ID" in
    let client_secret = get "OXFASTMCP_SERVER_AUTH_GOOGLE_CLIENT_SECRET" in
    let base_url = get "OXFASTMCP_SERVER_AUTH_GOOGLE_BASE_URL" in
    let issuer_url = get "OXFASTMCP_SERVER_AUTH_GOOGLE_ISSUER_URL" in
    let redirect_path = get "OXFASTMCP_SERVER_AUTH_GOOGLE_REDIRECT_PATH" in
    let required_scopes =
      parse_scopes (get "OXFASTMCP_SERVER_AUTH_GOOGLE_REQUIRED_SCOPES")
    in
    let timeout_seconds =
      match get "OXFASTMCP_SERVER_AUTH_GOOGLE_TIMEOUT_SECONDS" with
      | Some s -> Int.of_string_opt s
      | None -> None
    in
    let allowed_client_redirect_uris =
      parse_scopes
        (get "OXFASTMCP_SERVER_AUTH_GOOGLE_ALLOWED_CLIENT_REDIRECT_URIS")
    in
    let jwt_signing_key = get "OXFASTMCP_SERVER_AUTH_GOOGLE_JWT_SIGNING_KEY" in
    {
      client_id;
      client_secret;
      base_url;
      ...
      Error
        (Error.of_string
           "client_id is required - set via parameter or \
            OXFASTMCP_SERVER_AUTH_GOOGLE_CLIENT_ID")
      ...
      Error
        (Error.of_string
           "client_secret is required - set via parameter or \
            OXFASTMCP_SERVER_AUTH_GOOGLE_CLIENT_SECRET")
...
(** Google token verifier configuration.
...
module Google_token_verifier = struct
  ...
  (** Default User-Agent header for Google API requests *)
...
(** Google OAuth provider for OxFastMCP.
    ...
    OxFastMCP server. Just provide your Google OAuth app credentials and a base
...
module Google_provider = struct
  type t = {
    ...
    allowed_client_redirect_uris : string list option;
    jwt_signing_key : string option;
    token_verifier : Google_token_verifier.t;
    extra_authorize_params : (string * string) list;
  }
  [@@deriving sexp, compare]

  (** Default extra_authorize_params for Google OAuth. access_type=offline
      ensures refresh tokens are returned. prompt=consent forces consent screen
      to get refresh token. *)
  let default_extra_authorize_params =
  ...
  let create ?client_id ?client_secret ?base_url ?issuer_url ?redirect_path
    ...
    | Ok () ->
      ...
      in
      (* Google requires at least one scope - openid is the minimal OIDC
         scope *)
      ...
      (* Merge user-provided extra_authorize_params with Google defaults User
         params override defaults *)
      let extra_authorize_params_final =
        let user_keys =
          List.map extra_authorize_params ~f:fst |> Set.of_list (module String)
        in
        let filtered_defaults =
          List.filter default_extra_authorize_params ~f:(fun (k, _) ->
              not (Set.mem user_keys k))
        in
        filtered_defaults @ extra_authorize_params
      in

      (* Create token verifier *)
      let token_verifier =
        Google_token_verifier.create ~required_scopes:required_scopes_final
  ...
  (** Get the extra authorize params *)
  let extra_authorize_params t = t.extra_authorize_params

  (** Get the Google authorization endpoint *)
  let authorization_endpoint () = google_authorization_endpoint

  (** Get the Google token endpoint *)
  let token_endpoint () = google_token_endpoint
end
