(** Resource types for OxFastMCP
    ...
    Follows the same pattern as tool_types.ml and prompt_types.ml
...
type resource_kind =
  | Function_resource of {
      fn : reader;  (** Dynamic resource via function *)
    }
  | Text_resource of {
      content : string;  (** Static text content *)
    }
  | Binary_resource of {
      content : bytes;  (** Static binary content *)
    }
  | File_resource of {
...
(** Set resource key *)
let with_key (resource : t) (new_key : string) : t = Components.with_key resource new_key

(** {1 Resource Reading} *)

(** Read resource content *)
let read (resource : t) : (content, Ox_fast_mcp.Exceptions.error_data) Deferred.Result.t =
  if not resource.enabled then
    Deferred.return
      (Error
         {
           Ox_fast_mcp.Exceptions.message = Printf.sprintf "Resource '%s' is disabled" resource.name;
           code = Some 403;
           data = None;
         })
  else
    ...
    | File_resource { path } ->
      ...
      Monitor.try_with (fun () ->
        ...
        >>| fun contents -> Ok (Text contents))
      >>| function
      | Ok result -> result
      | Error exn ->
        Error {
          Ox_fast_mcp.Exceptions.message = Printf.sprintf "Failed to read file '%s': %s" path (Exn.to_string exn);
          code = Some 500;
          data = None;
        }
...
let create_function_resource ~uri ~name ?description ?(mime_type = "text/plain") 
  ...
  let resource_component_data = { data = resource_data; kind = Function_resource { fn } } in
  Components.create
    ~name
    ?description
    ...
    ?key
    ~enabled
    ~data:resource_component_data
    ()

(** Create a text resource *)
let create_text_resource ~uri ~name ~content ?description ?(mime_type = "text/plain")
    ?(tags = []) ?key ?annotations ?(enabled = true) () : t =
  let resource_data = { uri; mime_type; annotations } in
  let resource_component_data = { data = resource_data; kind = Text_resource { content } } in
  Components.create
    ~name
    ?description
    ~tags:(String.Set.of_list tags)
...
let create_binary_resource ~uri ~name ~content ?description ~mime_type
    ?(tags = []) ?key ?annotations ?(enabled = true) () : t =
  let resource_data = { uri; mime_type; annotations } in
  let resource_component_data = { data = resource_data; kind = Binary_resource { content } } in
  Components.create
    ~name
    ?description
    ~tags:(String.Set.of_list tags)
...
let create_file_resource ~uri ~name ~path ?description ?(mime_type = "text/plain")
    ?(tags = []) ?key ?annotations ?(enabled = true) () : t =
  let resource_data = { uri; mime_type; annotations } in
  let resource_component_data = { data = resource_data; kind = File_resource { path } } in
  Components.create
    ~name
    ?description
    ~tags:(String.Set.of_list tags)
...
let to_mcp_resource ?(_include_fastmcp_meta = false) (resource : t) : Mcp.Types.resource =
  ...
  } in
  {
    ...
    size = None;  (* TODO: Compute size if available *)
    annotations = None;  (* TODO: Add annotations if needed *)
    meta = None;  (* TODO: Add fastmcp metadata if requested *)
    base_metadata;
  }
...
let from_component (component : resource_component_data Components.component) : t = component
