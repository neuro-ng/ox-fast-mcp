...
let get_function_metadata = function
  | Normal (fn, sig_) ->
    let name =
      Stdlib.Obj.extension_constructor fn |> Stdlib.Obj.extension_name
    in
...
let extract_uri_parameters template =
  ...
  let rec find_all pos acc =
    ...
    with Not_found -> acc
  in
  find_all 0 [] |> List.rev |> String.Set.of_list

(** Validate function parameters against URI template *)
let validate_function_parameters ~uri_template ~required_params ~optional_params
    ~signature =
  ...
  if not (Set.is_subset required_params ~of_:uri_params) then (
    ...
    raise
      (Template_function_signature_error
         (sprintf
            "Required parameters %s must be present in URI template parameters \
             %s"
            (Set.to_list missing |> String.concat ~sep:", ")
            (Set.to_list uri_params |> String.concat ~sep:", ")));

    (* Check if URI parameters are a subset of all parameters unless we have
       kwargs *)
    let has_kwargs =
      List.exists signature.parameters ~f:(function
        | KwArgs -> true
        | _ -> false)
    in
    if not has_kwargs then
      ...
      if not (Set.is_subset uri_params ~of_:all_params) then
        ...
        raise
          (Template_function_signature_error
             (sprintf
                "URI parameters %s must be a subset of function parameters %s"
...
let build_regex template =
  ...
  let pattern =
    List.fold parts ~init:"" ~f:(fun acc part ->
        if
          String.is_prefix part ~prefix:"{" && String.is_suffix part ~suffix:"}"
        then
...
let to_mcp_template ?overrides t =
  ...
  let json =
    ...
    | None -> base
  in
  Mcp.Types.Resource_template.of_yojson (`Assoc json) |> function
  | Ok r -> r
...
let of_mcp_template (mcp_template : Mcp.Types.Resource_template.t) =
  {
    ...
    enabled = true;
    parameters = `Assoc [];
    (* Remote templates don't have local parameters *)
  }
...
let create_function_template ?name ?description ?mime_type ?tags
    ?(enabled = true) ~uri_template fn_type =
  (* Get function metadata *)
  let metadata = get_function_metadata fn_type in

  (* Extract parameters from function signature *)
  let required_params, optional_params =
    extract_parameters metadata.signature
  in
  ...
  (* Validate function parameters against URI template *)
  validate_function_parameters ~uri_template ~required_params ~optional_params
...
let read_function_template ?ctx t params =
  if not t.base.enabled then Lwt.fail_with "Template is disabled"
  else
      ...
      Lwt.fail
        (Parameter_validation_error
           (sprintf "Missing required parameters: %s"
              (Set.to_list missing |> String.concat ~sep:", ")))
    else t.fn ?ctx params

(** Create resource from template *)
let create_resource ?ctx t uri params =
  let* content = read_function_template ?ctx t params in
  let resource =
    Resource.make_function_resource ~uri:(Uri.of_string uri) ~name:t.base.name
      ?description:t.base.description ~mime_type:t.base.mime_type
      ~tags:t.base.tags ~enabled:t.base.enabled (fun () -> Lwt.return content)
  in
  Lwt.return resource
