(** Resource Manager for OxFastMCP
    ...
    Enhanced to align with Manager.S pattern and use new resource_types.
    See: specs/resource_manager.todo
*)
...
module DuplicateBehavior = struct
  ...
  let of_string = function
    ...
    | "error" -> Ok Error
    | "ignore" -> Ok Ignore
    | s -> Or_error.error_string (Printf.sprintf "Invalid duplicate_behavior: %s" s)
...
let create ?(duplicate_behavior = DuplicateBehavior.Warn) ?(mask_error_details = false) () : t =
  {
...
let list (manager : t) : Resource_types.t list =
  Hashtbl.data manager.resources

let list_enabled (manager : t) : Resource_types.t list =
  Hashtbl.data manager.resources
  |> List.filter ~f:Resource_types.is_enabled
...
let is_manager_enabled (manager : t) : bool =
  manager.enabled
...
let count (manager : t) : int =
  Hashtbl.length manager.resources
...
let find_by_name (manager : t) (name : string) : Resource_types.t option =
  Hashtbl.data manager.resources
  |> List.find ~f:(fun resource -> String.equal (Resource_types.get_name resource) name)
...
let read_resource (manager : t) (key : string) :
  ...
  if not manager.enabled then
    ...
    | None ->
      let error_data = {
        Ox_fast_mcp.Exceptions.message = Printf.sprintf "Resource not found: %s" key;
        code = Some 404;
        data = None;
      } in
      return (Error error_data)
    | Some resource ->
      if not (Resource_types.is_enabled resource) then
        let error_data = {
          Ox_fast_mcp.Exceptions.message = Printf.sprintf "Resource is disabled: %s" key;
          code = Some 403;
          data = None;
        } in
        return (Error error_data)
...
let get_resources (manager : t) : Resource_types.t list =
  list manager
