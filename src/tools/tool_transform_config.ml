...
let apply (cfg : t) (tool : Tool_types.t) : Tool_types.t =
  ...
  let transform_args_map =
    List.fold cfg.transform_args ~init:String.Map.empty ~f:(fun acc (arg_name, arg_cfg) ->
        Map.set acc ~key:arg_name ~data:(to_arg_transform arg_cfg))
  in
  Tool_types.create_transformed_tool
    tool
    ?name:cfg.name
    ?description:cfg.description
    ?tags:cfg.tags
