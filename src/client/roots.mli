...
(** Create a roots callback function from either a static list or a handler
    function.
    @param handler
      Either a static list of roots or a function that returns roots
    @return
      A function that takes a request context and returns a list_roots_result
    @raise Invalid_argument if the handler is invalid *)

val create_roots_callback_from_roots :
  roots_list list -> (request_context -> list_roots_result Lwt.t)
(** Internal function to create a callback from a static list of roots *)

val create_roots_callback_from_fn :
  roots_handler -> (request_context -> list_roots_result Lwt.t)
