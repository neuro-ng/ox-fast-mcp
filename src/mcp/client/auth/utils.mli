...
val extract_field_from_www_auth :
  Cohttp.Response.t -> string -> string option
(** Extract a named field from the WWW-Authenticate header using regex.
    
    [extract_field_from_www_auth response field_name] searches for the field
    in the WWW-Authenticate header and returns its value if found.
...
(** Extract protected resource metadata URL from WWW-Authenticate header per RFC9728.
...
(** Build ordered list of URLs for protected resource metadata discovery (SEP-985).
...
val build_oauth_authorization_server_metadata_discovery_urls :
  string option -> string -> string list
(** Build ordered list of URLs for OAuth authorization server metadata discovery.
    
    Discovery paths:
    - Legacy (no auth_server_url): [/.well-known/oauth-authorization-server] only
...
(** Handle client registration response.
    ...
    @raise Exceptions.Oauth_registration_error if status not 200/201 or invalid JSON *)
...
(** Build POST request for client registration.
    ...
    Uses [registration_endpoint] from metadata if available,
    otherwise falls back to [/register] on auth_base_url.
