...
let extract_field_from_www_auth response field_name =
  ...
  | Some www_auth_header ->
    ...
    (match Re.exec_opt pattern www_auth_header with
     | None -> None
     | Some groups ->
       (* Return quoted value if present (group 1), otherwise unquoted value (group 2) *)
       (try Some (Re.Group.get groups 1)
        with
        | _ -> (try Some (Re.Group.get groups 2) with | _ -> None)))
;;

let extract_scope_from_www_auth response = extract_field_from_www_auth response "scope"
...
let build_protected_resource_metadata_discovery_urls www_auth_url server_url =
  ...
  (match path with
   ...
   | p ->
     let path_based_url =
       Uri.with_path (Uri.of_string base_url) ("/.well-known/oauth-protected-resource" ^ p)
       |> Uri.to_string
     in
     urls := path_based_url :: !urls);
  (* Priority 3: Root-based well-known URI *)
  let root_based_url =
    Uri.with_path (Uri.of_string base_url) "/.well-known/oauth-protected-resource"
    |> Uri.to_string
  in
...
let build_oauth_authorization_server_metadata_discovery_urls auth_server_url server_url =
      ...
      | "" | "/" ->
        ...
        urls
          := (Uri.with_path
             ...
             :: !urls;
        urls
          := (Uri.with_path (Uri.of_string base_url) "/.well-known/openid-configuration"
              |> Uri.to_string)
             :: !urls
      | p ->
...
let get_client_metadata_scopes
  ~www_authenticate_scope
  ~protected_resource_metadata
  ?(authorization_server_metadata: Mcp_shared.Auth.oauth_metadata option = None)
  ()
  =
    ...
    (match protected_resource_metadata with
     | Some prm ->
       (match prm.Mcp_shared.Auth.scopes_supported with
        | Some scopes -> Some (String.concat ~sep:" " scopes) (* Priority 2: PRM scopes_supported *)
        | None ->
...
let is_valid_client_metadata_url url =
  ...
  | Some u ->
    (try
       ...
       let path = Uri.path parsed in
       String.( = ) scheme "https" && not (String.is_empty path || String.( = ) path "/")
     with
     | _ -> false)
;;
...
let create_client_info_from_metadata_url ~client_metadata_url ?(redirect_uris = []) () =
  ...
  | `OK ->
    ...
    >>| (fun content ->
          try
            ...
            let metadata = Mcp_shared.Auth.protected_resource_metadata_of_yojson json in
            Some metadata
          with
          | _ -> None)
  | _ -> return None
...
let handle_auth_metadata_response response body =
  ...
  | 200 ->
    ...
    >>| (fun content ->
          ...
          with
          | _ -> true, None)
  | c when c < 400 || c >= 500 -> return (false, None) (* Non-4XX error, stop trying *)
  | _ -> return (true, None)
;;
...
let handle_registration_response response body =
  ...
  then
    ...
    (try
       let json = Yojson.Safe.from_string content in
       let client_info = Mcp_shared.Auth.oauth_client_information_full_of_yojson json in
       return client_info
     with
     | exn -> raise (Exceptions.Oauth_registration_error (Exn.to_string exn)))
  else
    Cohttp_async.Body.to_string body
    >>= fun text ->
    raise
...
let handle_token_response_scopes _response body =
  ...
  with
  | exn -> raise (Exceptions.Oauth_token_error (Exn.to_string exn))
;;
...
let create_oauth_metadata_request url =
  ...
  let headers =
    Cohttp.Header.init ()
    |> fun h -> Cohttp.Header.add h mcp_protocol_version_header Mcp.Types.latest_protocol_version
  in
  Cohttp.Request.make ~meth:`GET ~headers uri
;;

let create_client_registration_request ~auth_server_metadata ~client_metadata ~auth_base_url
  =
  let registration_url =
    ...
    | Some asm ->
      (match asm.Mcp_shared.Auth.registration_endpoint with
       ...
       | None -> Uri.with_path (Uri.of_string auth_base_url) "/register" |> Uri.to_string)
    | None -> Uri.with_path (Uri.of_string auth_base_url) "/register" |> Uri.to_string
  in
  ...
  let headers =
    Cohttp.Header.init () |> fun h -> Cohttp.Header.add h "Content-Type" "application/json"
  in
  let _body = Cohttp_async.Body.of_string json_string in
  (* Note: Cohttp.Request.make doesn't directly support body, but we return it for the caller to use *)
