...
let is_initialization_request message =
  match message with
  | `Request (req : Types.jsonrpc_request) -> String.equal req.method_ "initialize"
  | _ -> false
...
let maybe_extract_session_id_from_response t response =
  ...
  | None ->
    Logs.debug (fun m -> m "No session ID in response headers")
...
let handle_sse_event t sse_event read_writer ~original_request_id:_
  ...
  | _ ->
    ...
    Logs.debug (fun m ->
        m "Ignoring SSE event type: %s" sse_event.event_type);
    return false
...
let handle_get_stream t read_writer =
  ...
  | None ->
    ...
    return ()
  | Some sid ->
    Logs.info (fun m -> m "Starting GET stream for server messages (session: %s)" sid);
    ...
    let cohttp_headers =
      let base_headers =
        Cohttp.Header.init ()
        |> fun h -> Cohttp.Header.add h accept_header sse_content_type
        |> fun h -> Cohttp.Header.add h mcp_session_id sid
      in
    ...
    try_with (fun () ->
        Cohttp_async.Client.get ~headers:cohttp_headers uri >>= fun (resp, body) ->
        let status_code =
        ...
        if status_code >= 200 && status_code < 300 then (
          ...
          Pipe.iter event_stream ~f:(fun event ->
              let%bind _is_complete =
                handle_sse_event t event read_writer
                  ~original_request_id:None ~is_initialization:false
              in
              return ()))
        else (
...
let handle_json_response t response read_writer ~is_initialization =
  ...
  Logs.debug (fun m -> m "Received JSON response: %s" (String.prefix body_str 100));
  ...
  | Ok () -> return ()
  | Error exn ->
    Logs.err (fun m -> m "Failed to parse JSON response: %s" (Exn.to_string exn));
    Pipe.write read_writer (`Error (Error.Streamable_http_error "JSON parse error"))
  ...
  let rec process_events () =
    ...
    | `Ok event -> (
      ...
      | _ ->
        (* Ignore other event types *)
        Logs.debug (fun m ->
            m "Ignoring SSE event type: %s" event.event_type);
        process_events ())
  in
...
let handle_post_request t ctx read_writer =
  let { headers; session_id = _; session_message; sse_read_timeout = _ } = ctx in
...
let terminate_session t =
  ...
  | Some sid ->
    ...
    | Error exn ->
      Logs.err (fun m ->
          m "Error terminating session: %s" (Exn.to_string exn));
      t.session_id <- None;
      return ()
