...
val send_progress_notification :
  t -> string -> float -> ?total:float -> ?message:string -> unit -> unit Lwt.t
...
val list_resource_templates :
  t -> ?cursor:string -> unit -> list_resource_templates_result Lwt.t
...
(** Start the session message handling loop. Must be called after
    initialization. *)
