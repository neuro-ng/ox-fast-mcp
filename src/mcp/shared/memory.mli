(** In-memory transports for client-server communication.
    ...
    Provides bi-directional pipe infrastructure for running MCP client
    and server in the same process. Useful for testing and embedding. *)
...
val create_client_server_memory_streams : unit -> message_stream * message_stream
