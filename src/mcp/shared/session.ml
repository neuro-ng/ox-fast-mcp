...
module Request_responder = struct
  ...
  let[@ocaml.warning "-16"] create ~request_id ?request_meta ~request ?message_metadata ~on_complete:_ =
...
module Base_session = struct
  ...
  let[@ocaml.warning "-27"] send_request
    ...
    (* We don't have a concrete way to decode 'recv_req here. Fail explicitly to match interface. *)
  ...
  let handle_incoming
      ...
      (_evt : [> `Request of (recv_req, send_res) Request_responder.t
