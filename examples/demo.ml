(** Comprehensive Demo MCP Server
    ...
    Demonstrates multiple OxFastMCP features including tools and resources. Run
    with: dune exec demo *)
    ...
    ~handler:(fun params ->
      ...
      | `Assoc fields -> (
        let name =
          ...
          |> Option.value ~default:(`String "World")
        in
        match name with
        | `String n ->
          return (`Assoc [ ("message", `String (sprintf "Hello, %s!" n)) ])
