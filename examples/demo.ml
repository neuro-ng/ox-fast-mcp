...
let demo_client_server () =
  ...
  Fast_mcp_server.register_resource server
    ...
    ~func:(fun () -> Mcp_types.String "This is a demo MCP server");
  
  (* Register a prompt *)
  Fast_mcp_server.register_prompt server
    ~name:"hello"
    ~description:"Hello prompt"
    ~func:(fun _params -> "Say hello!");
  ...
  (match Mcp_client.list_tools client with
   | Ok tools ->
     Printf.printf "✅ Available tools: %s\n"
       (String.concat ", " (List.map (fun (t : Mcp_types.tool_def) -> t.name) tools))
  ...
  (match Mcp_client.list_resources client with
   | Ok resources ->
     Printf.printf "✅ Available resources: %s\n"
       (String.concat ", " (List.map (fun (r : Mcp_types.resource_def) -> r.uri) resources))
   | Error err -> Printf.printf "❌ Failed to list resources: %s\n" err);
  
  (* Call a tool *)
  (match Mcp_client.call_tool client "greet" (Mcp_types.String "test") with
   | Ok (Mcp_types.Text result) -> Printf.printf "✅ Tool call result: %s\n" result
  ...
  (match Mcp_client.read_resource client "demo://info" with
   | Ok (Mcp_types.Text content) -> Printf.printf "✅ Resource content: %s\n" content
