...
let current_state =
  { temperature = 72.0; target_temperature = 70.0; mode = Mode.Off }
...
let () =
  FastMCP.add_simple_tool thermostat_mcp ~name:"get_thermostat_state"
    ...
    ~handler:(fun _ -> return (yojson_of_state current_state))
...
let set_target_temperature_schema =
  `Assoc
    [
      ("type", `String "object");
      ( "properties",
        `Assoc [ ("target", `Assoc [ ("type", `String "number") ]) ] );
...
let () =
  FastMCP.add_simple_tool thermostat_mcp ~name:"set_target_temperature"
    ...
    ~parameters:set_target_temperature_schema ~handler:(fun args ->
      ...
      | None ->
        return
          (`Assoc
            [
              ("error", `String "Missing required argument: target");
              ("success", `Bool false);
            ])
      | Some target ->
        ...
        return
          (`Assoc
            [ ("success", `Bool true); ("target_temperature", `Float target) ]))
            ...
            ( "mode",
              `Assoc
                [
                  ("type", `String "string");
                  ( "enum",
                    `List [ `String "off"; `String "heat"; `String "cool" ] );
                ] );
          ] );
...
let () =
  FastMCP.add_simple_tool thermostat_mcp ~name:"set_mode"
    ...
    ~parameters:set_mode_schema ~handler:(fun args ->
      ...
      | None ->
        return
          (`Assoc
            [
              ("error", `String "Missing required argument: mode");
              ("success", `Bool false);
            ])
      | Some mode_str -> (
        ...
        | Some m ->
          ...
          return
            (`Assoc
              [ ("success", `Bool true); ("mode", `String (Mode.to_string m)) ])
        | None ->
          return
            (`Assoc
              [ ("success", `Bool false); ("error", `String "Invalid mode") ])))
