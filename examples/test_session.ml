...
let main () =
  let process = Process.create_exn 
    ~prog:"/home/neu/workspace/ox-fast-mcp/debug_calculator.sh" 
    ~args:[] 
    ()
  in
  ...
  (* 1. Initialize *)
  let init_req = "{\"jsonrpc\": \"2.0\", \"id\": 0, \"method\": \"initialize\", \"params\": {\"protocolVersion\": \"2024-11-05\", \"capabilities\": {}, \"clientInfo\": {\"name\": \"test\", \"version\": \"1.0\"}}}" in
  printf "Sending: %s\n" init_req;
  Writer.write_line writer init_req;
  let%bind () = read_response () in

  (* 2. Initialized *)
  let initialized = "{\"jsonrpc\": \"2.0\", \"method\": \"notifications/initialized\"}" in
  printf "Sending: %s\n" initialized;
  Writer.write_line writer initialized;
  
  (* 3. List Tools *)
  let tools_req = "{\"jsonrpc\": \"2.0\", \"id\": 1, \"method\": \"tools/list\"}" in
  printf "Sending: %s\n" tools_req;
  Writer.write_line writer tools_req;
  let%bind () = read_response () in

  (* 4. Call Tool *)
  let call_req = "{\"jsonrpc\": \"2.0\", \"id\": 2, \"method\": \"tools/call\", \"params\": {\"name\": \"add\", \"arguments\": {\"a\": 4, \"b\": 6}}}" in
  printf "Sending: %s\n" call_req;
