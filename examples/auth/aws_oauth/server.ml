...
let run () =
  let%bind provider_result =
    Server_auth_providers.Aws.Aws_cognito_provider.create () |> return
  in
  (* 1. Fetch OIDC Configuration *)
  let%bind provider, jwks_uri =
    match provider_result with
    | Error e ->
      Logging.Logger.error
        (Logging.Logger.get_logger "aws_oauth_server")
        (sprintf "Failed to initialize AWS Cognito provider: %s"
           (Error.to_string_hum e));
      exit 1
    | Ok provider -> (
      Logging.Logger.info
        (Logging.Logger.get_logger "aws_oauth_server")
        (sprintf "Fetching OIDC configuration from %s" provider.config_url);
      match%bind
        try_with (fun () ->
            let%bind _, body = Client.get (Uri.of_string provider.config_url) in
            let%bind body_str = Body.to_string body in
            let json = Yojson.Safe.from_string body_str in
            let jwks_uri =
              Yojson.Safe.Util.(member "jwks_uri" json |> to_string)
            in
            return jwks_uri)
      with
      | Ok uri -> return (provider, Some uri)
      | Error exn ->
        Logging.Logger.error
          (Logging.Logger.get_logger "aws_oauth_server")
          (sprintf "Failed to fetch OIDC configuration: %s" (Exn.to_string exn));
        return (provider, None))
  in

  let server =
    Ox_fast_mcp_server.Server.Ox_fast_mcp.create
      ~name:"AWS Cognito OAuth Example Server" ()
  in
  ...
  let () =
    Server.Ox_fast_mcp.add_simple_tool server ~name:"get_access_token_claims"
      ...
      ~handler:(fun _args ->
        (* Context access is not directly supported in simple tools yet. For
           now, we return a placeholder or need to use a different mechanism. If
           Context is needed, we might rely on Middleware injection or Async
           locals if supported. For this example, we'll return a stub or try to
           find a way. *)
        return
          (`Assoc
            [
...
let () =
  Command.async ~summary:"AWS Cognito OAuth Example Server"
