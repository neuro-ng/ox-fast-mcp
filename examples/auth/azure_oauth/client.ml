...
let main () =
  ...
  let transport_result =
    Transports.create_streamable_http_transport ~auth:OAuth
      (* Note: For full OAuth flow (opening browser), we rely on the transport
         or external helper. The Python client uses 'auth="oauth"' which implies
         generic handling. Here we initialize with empty token placeholder or
         expect it to trigger flow. *)
  ...
  | Ok transport -> (
    ...
    (* 3. Ping *)
    print_endline "🔌 Connecting to Azure OAuth Protected Server...";
...
let () =
  Command.async ~summary:"Azure OAuth Client Example"
