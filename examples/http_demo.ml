...
let () =
  Command.async ~summary:"Start HTTP server with SSE transport"
    (Command.Param.return (fun () ->
         ...
         (* Configure server *)
         let config =
           Http.Server_config.{ host = "0.0.0.0"; port = 8000; backlog = 10 }
         in

         let%bind () =
           Log.Global.info "Starting HTTP server on http://%s:%d" config.host
             config.port;
           Deferred.unit
         in
         ...
         let%bind () =
           Log.Global.info "Server running. SSE endpoint: http://%s:%d/sse"
