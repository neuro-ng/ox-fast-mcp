...
let find_available_port () =
  ...
  let sock = Unix.socket ~domain:Unix.PF_INET ~kind:Unix.SOCK_STREAM ~protocol:0 in
  Unix.bind sock addr;
  let _, port = match Unix.getsockname sock with
    | Unix.ADDR_INET (_, port) -> Unix.close sock; (Unix.Inet_addr.localhost, port)
    | _ -> Unix.close sock; failwith "Expected INET socket" in
  port
