...
let test_with_server _ =
  ...
  with_server (fun url ->
      test_url := url;
      assert_bool "URL should start with http://"
        (String.is_prefix ~prefix:"http://" url);
      assert_bool "URL should contain localhost"
        (String.is_substring ~substring:"127.0.0.1" url);
      true)
  |> ignore;
