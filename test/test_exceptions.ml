...
(* Import the exceptions module - equivalent to 'from oxfastmcp import
   exceptions' *)
...
let%expect_test "test error_data creation and serialization" =
  ...
  require_equal ~here:[%here]
    (module struct
      type t = int option [@@deriving compare, sexp_of, equal]
    end)
    error_data.code (Some 500);
...
let%expect_test "test error_data with data field" =
  ...
  (* Test round-trip *)
  let deserialized = error_data_of_yojson json in
  require_equal ~here:[%here] (module String) deserialized.message
    "Error with data";
  require_equal ~here:[%here]
    (module struct
      type t = int option [@@deriving compare, sexp_of, equal]
    end)
    deserialized.code None;
  return ()

let%expect_test "test OxFastMCP_error exception" =
  let error_data =
    { message = "OxFastMCP test error"; code = Some 400; data = None }
  in
  ...
  require_equal ~here:[%here] (module String) extracted_data.message
    "OxFastMCP test error";
  require_equal ~here:[%here]
    (module struct
      type t = int option [@@deriving compare, sexp_of, equal]
    end)
    extracted_data.code (Some 400);

  (* Test string conversion *)
  let error_string = to_string exception_val in
  require ~here:[%here]
    (String.is_substring error_string ~substring:"OxFastMCP_error");
  require ~here:[%here]
    (String.is_substring error_string ~substring:"OxFastMCP test error");
...
let%expect_test "test all exception types" =
  let test_exception_type name create_fn exception_constructor =
    let error_data =
      { message = sprintf "%s test" name; code = Some 123; data = None }
    in
    ...
    require_equal ~here:[%here] (module String) helper_extracted.message
      (sprintf "%s helper test" name)
  in

  test_exception_type "OxFastMCP_error" create_oxfastmcp_error (fun d ->
      OxFastMCP_error d);
  [%expect {| "OxFastMCP_error: OxFastMCP_error test" |}];

  test_exception_type "Validation_error" create_validation_error (fun d ->
      Validation_error d);
  [%expect {| "Validation_error: Validation_error test" |}];

  test_exception_type "Resource_error" create_resource_error (fun d ->
      Resource_error d);
  ...
  test_exception_type "Prompt_error" create_prompt_error (fun d ->
      Prompt_error d);
  [%expect {| "Prompt_error: Prompt_error test" |}];

  test_exception_type "Invalid_signature" create_invalid_signature (fun d ->
      Invalid_signature d);
  [%expect {| "Invalid_signature: Invalid_signature test" |}];

  test_exception_type "Client_error" create_client_error (fun d ->
      Client_error d);
  [%expect {| "Client_error: Client_error test" |}];

  test_exception_type "Not_found_error" create_not_found_error (fun d ->
      Not_found_error d);
  [%expect {| "Not_found_error: Not_found_error test" |}];

  test_exception_type "Disabled_error" create_disabled_error (fun d ->
      Disabled_error d);
...
let%expect_test "test helper functions with optional parameters" =
  ...
  let error_with_code =
    create_resource_error ~code:(Some 404) "Resource not found"
  in
  let extracted_with_code = get_error_data error_with_code in
  require_equal ~here:[%here] (module String) extracted_with_code.message
    "Resource not found";
  require_equal ~here:[%here]
    (module struct
      type t = int option [@@deriving compare, sexp_of, equal]
    end)
    extracted_with_code.code (Some 404);

  (* Test helper functions with optional data *)
  let json_data = `Assoc [ ("details", `String "Additional info") ] in
  let error_with_data =
    create_tool_error ~data:(Some json_data) "Tool execution failed"
  in
  let extracted_with_data = get_error_data error_with_data in
  require_equal ~here:[%here] (module String) extracted_with_data.message
    "Tool execution failed";
  require ~here:[%here] (Option.is_some extracted_with_data.data);

  (* Test helper functions with both optional parameters *)
  let full_error =
    create_client_error ~code:(Some 500) ~data:(Some json_data)
      "Client connection error"
  in
  let extracted_full = get_error_data full_error in
  require_equal ~here:[%here] (module String) extracted_full.message
    "Client connection error";
  require_equal ~here:[%here]
    (module struct
      type t = int option [@@deriving compare, sexp_of, equal]
    end)
    extracted_full.code (Some 500);
...
let%expect_test "test exception pattern matching" =
  let test_pattern_matching exception_val expected_type =
    ...
    | OxFastMCP_error _ ->
      require_equal ~here:[%here] (module String) expected_type
        "OxFastMCP_error"
    | Validation_error _ ->
      require_equal ~here:[%here] (module String) expected_type
        "Validation_error"
    | Resource_error _ ->
      require_equal ~here:[%here] (module String) expected_type "Resource_error"
    | Tool_error _ ->
      require_equal ~here:[%here] (module String) expected_type "Tool_error"
    | Prompt_error _ ->
      require_equal ~here:[%here] (module String) expected_type "Prompt_error"
    | Invalid_signature _ ->
      require_equal ~here:[%here] (module String) expected_type
        "Invalid_signature"
    | Client_error _ ->
      require_equal ~here:[%here] (module String) expected_type "Client_error"
    | Not_found_error _ ->
      require_equal ~here:[%here] (module String) expected_type
        "Not_found_error"
    | Disabled_error _ ->
      require_equal ~here:[%here] (module String) expected_type "Disabled_error"
...
let%expect_test "test edge cases and error conditions" =
  ...
  require_equal ~here:[%here] (module String) extracted_long.message
    long_message;

  (* Test zero code *)
  let zero_code_error =
    create_resource_error ~code:(Some 0) "Zero code error"
  in
  ...
  require_equal ~here:[%here]
    ...
    extracted_zero.code (Some 0);

  (* Test negative code *)
  let neg_code_error =
    create_tool_error ~code:(Some (-1)) "Negative code error"
  in
...
let%expect_test "test JSON edge cases" =
  ...
  let complex_error =
    create_prompt_error ~data:(Some complex_json) "Complex JSON error"
  in
...
let%expect_test "test string representation consistency" =
  ...
  List.iter test_cases ~f:(fun (expected_prefix, create_fn) ->
      ...
      require ~here:[%here]
        (String.is_prefix error_string ~prefix:expected_prefix);
      require ~here:[%here]
        (String.is_substring error_string ~substring:"Test message"));
