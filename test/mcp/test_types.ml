...
let%expect_test "task_status sexp" =
  ...
  in
  List.iter statuses ~f:(fun status -> print_s [%sexp (status : task_status)]);
  [%expect
    {|
    Working
    Input_required
...
let%expect_test "task type construction" =
  ...
  print_s
    [%sexp
      {
        task_id = (task.task_id : string);
        status = (task.status : task_status);
      }];
  [%expect {| ((task_id task-123) (status Working)) |}]

let%expect_test "get_task_request sexp" =
  let params : get_task_request_params =
    { task_id = "task-456"; request_params = { meta = None } }
  in
  let request : get_task_request = { method_ = "tasks/get"; params } in
  print_s
    [%sexp
      {
...
let%expect_test "cancel_task_request sexp" =
  let params : cancel_task_request_params =
    { task_id = "task-789"; request_params = { meta = None } }
  in
  let request : cancel_task_request = { method_ = "tasks/cancel"; params } in
  print_s
    [%sexp
      {
        method_ = (request.method_ : string);
...
let%expect_test "sampling_message construction" =
  let text_content : text_content =
    { type_ = `Text; text = "Hello, world!"; annotations = None; meta = None }
  in
  let message : sampling_message =
    { role = `User; content = `Text text_content; meta = None }
  in
...
let%expect_test "tool_use_content sexp" =
  ...
  in
  print_s [%sexp { name = (content.name : string); id = (content.id : string) }];
...
let%expect_test "stop_reason sexp" =
  let reasons : stop_reason list =
    [ `EndTurn; `StopSequence; `MaxTokens; `String "custom" ]
  in
  List.iter reasons ~f:(fun reason -> print_s [%sexp (reason : stop_reason)]);
  [%expect
    {|
    EndTurn
    StopSequence
...
(* Note: Elicitation schema types are defined in types.ml but not exported in
   types.mli, so we skip testing those internal types here. *)
...
let%expect_test "client_request includes task types" =
  ...
  in
  let cancel_task : client_request =
    `CancelTask
      {
...
let%expect_test "server_notification includes TaskStatus" =
  let task_notif : server_notification =
    `TaskStatus
      {
