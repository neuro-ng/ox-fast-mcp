                              ...
                              [
                                `Assoc
                                  [
                                    ...
                                    ("in", `String "query");
                                    ( "description",
                                      `String "Number of items to skip" );
                                    ("required", `Bool false);
                                    ( "schema",
                                ...
                                `Assoc
                                  [
                                    ...
                                    ("in", `String "query");
                                    ( "description",
                                      `String "Max number of items to return" );
                                    ("required", `Bool false);
                                    ( "schema",
                                      `Assoc
                                        [
                          ...
                          ( "responses",
                            `Assoc
                              [
                                ( "201",
                                  `Assoc
                                    [ ("description", `String "Item created") ]
                                );
                              ] );
                        ] );
                  ] );
...
let test_parse_fastapi_schema_operation_ids () =
  let routes = parse_openapi_to_http_routes TestData.fastapi_schema in
  let expected_operations = [ "list_items"; "create_item" ] in
  List.iter
    (fun op_id ->
      let found =
        List.exists
...
let test_path_parameter_parsing () =
  let routes = parse_openapi_to_http_routes TestData.fastapi_schema in
  (* Since the current schema doesn't have path parameters, we'll test that
     there are 0 path params *)
  let list_items =
    List.find_opt
      (fun route ->
        match route.operation_id with
  ...
  | Some route ->
    let path_params =
      List.filter (fun param -> param.location = `Path) route.parameters
    in
...
let test_query_parameter_parsing () =
  ...
  | Some route ->
    let query_params =
      List.filter (fun param -> param.location = `Query) route.parameters
    in
...
let test_header_parameter_parsing () =
  ...
  | Some route ->
    let header_params =
      List.filter (fun param -> param.location = `Header) route.parameters
    in
...
let test_request_body_content_type () =
  ...
  | Some route -> (
    ...
    | Some body ->
      check bool "has application/json"
        (List.mem_assoc "application/json" body.content_schema)
        true)
  ...
  [
    ( "FastAPI OpenAPI Schema",
      [
        ...
        test_case "Request body content type" `Quick
          test_request_body_content_type;
      ] );
  ]
