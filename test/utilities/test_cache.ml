...
let%expect_test "test_init" =
  ...
  print_s
    [%sexp (Cache.TimedCache.get_expiration cache : Time_float_unix.Span.t)];
...
let%expect_test "test_set_and_get" =
  let%bind () = Clock.after (Time_float_unix.Span.of_sec 0.0) in
  let cache =
    Cache.TimedCache.create ~expiration:(Time_float_unix.Span.of_sec 10.0)
  in
...
let%expect_test "test_get_not_found" =
  ...
  in
  print_s
    [%sexp (Cache.TimedCache.get cache ~key:"nonexistent" : string option)];
  [%expect {| () |}];
  return ()
...
let%expect_test "test_expiration" =
  let%bind () = Clock.after (Time_float_unix.Span.of_sec 0.0) in
  let cache =
    Cache.TimedCache.create ~expiration:(Time_float_unix.Span.of_sec 1.0)
  in
  Cache.TimedCache.set cache ~key:"test" ~value:"value";
...
let%expect_test "test_many_items" =
  ...
  List.iter (List.range 0 1000) ~f:(fun i ->
      Cache.TimedCache.set cache ~key:(Int.to_string i)
        ~value:(sprintf "value%d" i));
  print_s [%sexp (Cache.TimedCache.size cache : int)];
  List.iter [ 0; 123; 456; 789; 999 ] ~f:(fun i ->
      print_s
        [%sexp
          (Cache.TimedCache.get cache ~key:(Int.to_string i) : string option)]);
  [%expect
    {|
    1000
    (value0)
...
let%expect_test "test_different_key_types" =
  ...
  print_s
    [%sexp (Cache.TimedCache.get cache ~key:(Int.to_string 42) : string option)];

  (* Test float key *)
  Cache.TimedCache.set cache ~key:(Float.to_string 3.14) ~value:"float_value";
  print_s
    [%sexp
      (Cache.TimedCache.get cache ~key:(Float.to_string 3.14) : string option)];
...
let%expect_test "test_none_value" =
  ...
  let cache =
    Cache.TimedCache.create ~expiration:(Time_float_unix.Span.of_sec 10.0)
  in
  Cache.TimedCache.set cache ~key:"none_key" ~value:"";
...
let%expect_test "test_edge_case_zero_expiration" =
  let%bind () = Clock.after (Time_float_unix.Span.of_sec 0.0) in
  let cache =
    Cache.TimedCache.create ~expiration:(Time_float_unix.Span.of_sec 0.0)
  in
  Cache.TimedCache.set cache ~key:"test" ~value:"value";
