...
let%expect_test "normalize_tool_name_spaces_to_underscores" =
  ...
  let normalized =
    Ox_fast_mcp_server.Server.Ox_fast_mcp.normalize_tool_name name
  in
...
let%expect_test "add_tools_multiple" =
  let server = Ox_fast_mcp_server.Server.Ox_fast_mcp.create ~name:"test" () in
  let tool1 =
  ...
  let tool3 =
    Ox_fast_mcp_server.Server.Tool.create ~name:"tool3"
      ...
      ()
  in
  Ox_fast_mcp_server.Server.Ox_fast_mcp.add_tools server [ tool1; tool2; tool3 ];
  let tools = Ox_fast_mcp_server.Server.Ox_fast_mcp.get_tools server in
...
let%expect_test "add_resources_multiple" =
  let server = Ox_fast_mcp_server.Server.Ox_fast_mcp.create ~name:"test" () in
  let resource1 =
    Ox_fast_mcp_server.Server.Resource.create ~uri:"file:///test1" ~name:"r1"
      ~reader:(fun () -> return "content1")
      ()
  in
  let resource2 =
    Ox_fast_mcp_server.Server.Resource.create ~uri:"file:///test2" ~name:"r2"
      ~reader:(fun () -> return "content2")
      ()
  in
  Ox_fast_mcp_server.Server.Ox_fast_mcp.add_resources server
    [ resource1; resource2 ];
  let resources = Ox_fast_mcp_server.Server.Ox_fast_mcp.get_resources server in
...
let%expect_test "add_prompts_multiple" =
  ...
  let prompt2 =
    Ox_fast_mcp_server.Server.Prompt.create ~name:"prompt2"
      ...
      ()
  in
  Ox_fast_mcp_server.Server.Ox_fast_mcp.add_prompts server [ prompt1; prompt2 ];
  let prompts = Ox_fast_mcp_server.Server.Ox_fast_mcp.get_prompts server in
...
let%expect_test "is_valid_tool_name_valid_names" =
  ...
  List.iter valid_names ~f:(fun name ->
      let is_valid =
        Ox_fast_mcp_server.Server.Ox_fast_mcp.is_valid_tool_name name
      in
      printf "%s: %b\n" name is_valid);
  [%expect
    {|
...
let%expect_test "is_valid_uri_valid_uris" =
  ...
  List.iter valid_uris ~f:(fun uri ->
      let is_valid = Ox_fast_mcp_server.Server.Ox_fast_mcp.is_valid_uri uri in
      printf "%s: %b\n" uri is_valid);
  [%expect
    {|
...
let%expect_test "is_valid_prompt_name_valid" =
  ...
  List.iter valid_names ~f:(fun name ->
      let is_valid =
        Ox_fast_mcp_server.Server.Ox_fast_mcp.is_valid_prompt_name name
      in
      printf "%s: %b\n" name is_valid);
  [%expect {|
    my_prompt: true
...
let%expect_test "is_valid_template_uri_valid" =
  ...
  List.iter valid_uris ~f:(fun uri ->
      let is_valid =
        Ox_fast_mcp_server.Server.Ox_fast_mcp.is_valid_template_uri uri
      in
      printf "%s: %b\n" uri is_valid);
  [%expect
    {|
...
let%expect_test "normalize_then_validate" =
  ...
  let normalized =
    Ox_fast_mcp_server.Server.Ox_fast_mcp.normalize_tool_name invalid_name
  in
  let is_valid =
    Ox_fast_mcp_server.Server.Ox_fast_mcp.is_valid_tool_name normalized
  in
  printf "Original: %s\n" invalid_name;
  printf "Normalized: %s\n" normalized;
  printf "Is valid: %b\n" is_valid;
...
let%expect_test "batch_add_then_list" =
  ...
  let tools =
    List.init 5 ~f:(fun i ->
        Ox_fast_mcp_server.Server.Tool.create ~name:(sprintf "tool%d" i)
          ~handler:(fun _ -> return (`Int i))
          ())
  in
  Ox_fast_mcp_server.Server.Ox_fast_mcp.add_tools server tools;
  let tool_list = Ox_fast_mcp_server.Server.Ox_fast_mcp.list_tools_mcp server in
