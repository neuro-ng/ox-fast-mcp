...
let%expect_test "ProxyDCRClient - default allows all (DCR compatibility)" =
  (* Test that default configuration allows all URIs for DCR compatibility. When
     allowed_redirect_uri_patterns is None, any URI should be accepted. *)
  let client =
    {
      pdc_client_id = "test";
  ...
  let test uri =
    let result =
      Server_auth.Redirect_validation.validate_redirect_uri
  ...
  let client =
    {
      ...
      client_secret = Some "secret";
      redirect_uris = [ "http://localhost:3000" ];
      allowed_redirect_uri_patterns = Some [];
      (* Empty = allow none *)
      client_name = None;
      pdc_created_at = 0.0;
    }
...
let%expect_test "ProxyDCRClient - None redirect_uri uses default behavior" =
  ...
  let valid =
    Server_auth.Redirect_validation.validate_redirect_uri ~redirect_uri:None
...
let%expect_test "OAuthProxy - default allowed_client_redirect_uris is None" =
  (* Test that OAuth proxy defaults to allowing all URIs for DCR compatibility.
     ...
     Note: OCaml Oauth_proxy.t is abstract, so we can only test behavior, not
     internal field values. We test via validate_redirect_uri behavior. *)
  ...
  let result =
    validate_redirect_uri proxy ~client ~redirect_uri:"http://example.com"
  in
...
let%expect_test "OAuthProxy - custom patterns restrict validation" =
  ...
  let proxy =
    create_test_proxy ~allowed_client_redirect_uris:custom_patterns ()
  in
  let client =
    {
      pdc_client_id = "test";
...
let%expect_test "OAuthProxy - register and get client" =
  Lwt_main.run
    (let proxy =
       create_test_proxy
         ~allowed_client_redirect_uris:[ "https://app.example.com/*" ]
         ()
     in
     (* Register a client *)
     let* _registered =
