(** Tests for OAuth .well-known routes when OxFastMCP apps are mounted in parent
    apps.
    ...
    Note: Full HTTP integration tests require Starlette/ASGI equivalents which
    are not yet implemented in OCaml. This file tests the modules that ARE
    currently available and documents expected behavior via stub tests. *)
...
let%expect_test "Oauth_proxy - base_url with mount prefix in redirect" =
  ...
  in
  let url =
    build_upstream_authorize_url proxy ~txn_id:"test-txn" ~transaction
  in
...
(* ============================================================================
   RFC 9728 Well-Known Routes - Stub Tests These document expected behavior for
...
let%expect_test "[STUB] RFC 9728 - mounted app well-known path" =
  (* RFC 9728: If app mounted at /api with resource at /mcp, well-known endpoint
     should be at: /.well-known/oauth-protected-resource/api/mcp *)
...
let%expect_test "[STUB] Issue #2287 - endpoints at base_url not issuer_url" =
  (* Fix for issue #2287: When base_url and issuer_url differ, operational OAuth
     endpoints should be declared at base_url. *)
