...
let%expect_test "tool_lookup_with_suggestion" =
  ...
  | Error exn ->
      ...
      if String.is_substring error_str ~substring:"did_you_mean" then
        ...
        print_endline "Error missing suggestion");
...
let%expect_test "tool_lookup_no_suggestion" =
  ...
  | Error exn ->
      ...
      if String.is_substring error_str ~substring:"available" then
        ...
        print_endline "Basic error message");
...
let%expect_test "resource_lookup_with_suggestion" =
  ...
  let%bind result =
    Monitor.try_with (fun () ->
        get_resource server ~key:"file:///confg.json")
  in
  (match result with
  | Ok _ -> print_endline "Unexpected success"
  | Error exn ->
...
let%expect_test "prompt_lookup_with_suggestion" =
  let open Server.Ox_fast_mcp in
  let server = create ~name:"test" () in
  add_simple_prompt ~name:"write_email" 
    ~render:(fun _ -> return `Null) server;
  add_simple_prompt ~name:"write_story"
    ~render:(fun _ -> return `Null) server;
  
  let%bind result =
    Monitor.try_with (fun () ->
        get_prompt_component server ~key:"write_emai")
  in
  (match result with
  | Ok _ -> print_endline "Unexpected success"
  | Error exn ->
...
let%expect_test "template_lookup_with_suggestion" =
  let open Server.Ox_fast_mcp in
  let server = create ~name:"test" () in
  let template1 = Server.Resource_template.create
    ~uri_template:"file:///{path}/data"
    ~name:"file_template"
    ~create_resource:(fun ~params:_ ->
      return (Server.Resource.create ~uri:"file:///test" ~name:"test"
  ...
  let%bind result =
    Monitor.try_with (fun () ->
        get_template server ~key:"file:///{pth}/data")
  in
  (match result with
  | Ok _ -> print_endline "Unexpected success" 
  | Error exn ->
...
let%expect_test "multiple_similar_tools" =
  ...
  | Error exn ->
      ...
      if String.is_substring error_str ~substring:"did_you_mean" then
        ...
        print_endline "Error missing suggestions");
...
let%expect_test "exact_typo_correction" =
  ...
  | Error exn ->
      ...
      if String.is_substring error_str ~substring:"calculate_sum" then
        ...
        print_endline "Missing expected suggestion");
