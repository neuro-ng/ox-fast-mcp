...
let%expect_test "list_all_component_names_with_components" =
  ...
  | `Assoc fields ->
      ...
      let resources = List.Assoc.find_exn fields ~equal:String.equal "resources" in
...
let%expect_test "component_count_by_tag_with_tags" =
  ...
  let tool2 = Server.Tool.create ~name:"tool2"
    ...
    ~handler:(fun _ -> return `Null) () in
  let resource1 = Server.Resource.create ~uri:"file:///test" ~name:"res1"
  ...
  let counts = component_count_by_tag server in
  List.iter counts ~f:(fun (tag, count) ->
      printf "%s: %d\n" tag count);
  [%expect {|
    api: 3
    v2: 1
