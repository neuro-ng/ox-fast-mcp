...
let%expect_test "describe_server_basic_info" =
  ...
  let server = create ~name:"test-server" ~version:"1.0.0" 
    ~instructions:"Test instructions" () in
...
let%expect_test "describe_server_with_components" =
  ...
  add_simple_resource ~uri:"file:///test" ~name:"res1" 
    ~reader:(fun () -> return "data") server;
  add_simple_prompt ~name:"prompt1" ~render:(fun _ -> return (`String "rendered")) server;
  
  let description = describe_server server in
  (match description with
  | `Assoc fields ->
      let tool_count = List.Assoc.find_exn fields ~equal:String.equal "tool_count" in
      let resource_count = List.Assoc.find_exn fields ~equal:String.equal "resource_count" in
      let prompt_count = List.Assoc.find_exn fields ~equal:String.equal "prompt_count" in
...
let%expect_test "find_tools_by_tag" =
  ...
  let tool1 = Server.Tool.create ~name:"tool1" 
    ~tags:(String.Set.of_list ["api"; "v1"])
    ~handler:(fun _ -> return `Null) () in
  let tool2 = Server.Tool.create ~name:"tool2" 
    ~tags:(String.Set.of_list ["api"; "v2"])
    ~handler:(fun _ -> return `Null) () in
  let tool3 = Server.Tool.create ~name:"tool3" 
...
let%expect_test "find_resources_by_scheme" =
  let open Server.Ox_fast_mcp in
  let server = create ~name:"test" () in
  let file_res = Server.Resource.create ~uri:"file:///path/to/file" ~name:"file"
    ~reader:(fun () -> return "data") () in
  let http_res = Server.Resource.create ~uri:"http://example.com/api" ~name:"http"
    ~reader:(fun () -> return "data") () in
  let https_res = Server.Resource.create ~uri:"https://secure.com/data" ~name:"https"
    ~reader:(fun () -> return "data") () in
...
let%expect_test "find_prompts_by_tag" =
  let open Server.Ox_fast_mcp in
  let server = create ~name:"test" () in
  let prompt1 = Server.Prompt.create ~name:"prompt1"
    ~tags:(String.Set.of_list ["creative"])
    ~render:(fun _ -> return `Null) () in
  let prompt2 = Server.Prompt.create ~name:"prompt2"
    ~tags:(String.Set.of_list ["creative"; "long"])
    ~render:(fun _ -> return `Null) () in
  let prompt3 = Server.Prompt.create ~name:"prompt3"
...
let%expect_test "suggest_similar_names_exact_match" =
  ...
  let suggestions = Server.Ox_fast_mcp.suggest_similar_names "user_pro" available in
...
let%expect_test "suggest_similar_names_typo" =
  ...
  let suggestions = Server.Ox_fast_mcp.suggest_similar_names "calulate" available in
  List.iter suggestions ~f:print_endline;
  [%expect {|
    calculate
    calibrate
...
let%expect_test "suggest_similar_names_no_matches" =
  ...
  let suggestions = Server.Ox_fast_mcp.suggest_similar_names "zebra" available in
...
let%expect_test "validate_server_invalid_uri" =
  ...
  let invalid_res = Server.Resource.create ~uri:"no_scheme" ~name:"bad"
    ~reader:(fun () -> return "data") () in
...
let%expect_test "search_then_validate" =
  ...
  add_simple_tool ~name:"api_tool" 
    ~handler:(fun _ -> return `Null) server;
  let tool_with_tag = Server.Tool.create ~name:"tagged_tool"
  ...
  printf "%s: " "Found production tools"; print_s [%sexp (List.length prod_tools : int)];
  
  (* Validate server *)
  let validation = validate_server server in
  printf "%s: " "Server valid"; print_s [%sexp (Result.is_ok validation : bool)];
...
let%expect_test "suggest_on_missing_component" =
  ...
  add_simple_tool ~name:"calculate_product" ~handler:(fun _ -> return `Null) server;
