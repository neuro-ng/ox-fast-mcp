...
module Test_error_handling = struct
  ...
  let test_init_default _ =
    ...
    assert_bool "include_traceback should be false" (not middleware.include_traceback);
    assert_bool "error_callback should be None" (Option.is_none middleware.error_callback);
    assert_bool "transform_errors should be true" middleware.transform_errors;
    assert_bool "error_counts should be empty" (Map.is_empty middleware.error_counts)
  ...
  let test_init_custom _ =
    ...
    assert_bool "error_callback should be Some" (Option.is_some middleware.error_callback);
    assert_bool "transform_errors should be false" (not middleware.transform_errors)

  let test_log_error_basic _ =
    let middleware = create () in
    let error = Failure "test error" in
    let log_messages = ref [] in
    let logger = Logger.create ~on_log:(fun lvl msg -> log_messages := (lvl, msg) :: !log_messages) "test" in
    let middleware = { middleware with logger } in
    
    log_error middleware error mock_context;
    
    let has_error_message = List.exists !log_messages ~f:(fun (lvl, msg) ->
      lvl = Logger.Error && String.is_substring msg ~substring:"Error in test_method: Failure: test error"
    ) in
  ...
  let test_transform_error_mcp_error _ =
    let middleware = create () in
    let error = Mcp_error { code = -32001; message = "test error"; details = None } in
  ...
  let test_transform_error_file_not_found _ =
    ...
    | Mcp_error err ->
      ...
      assert_bool "Should contain error message"
        (String.is_substring err.message ~substring:"Resource not found: test error")
    | _ -> assert_failure "Should return MCP error"
  ...
  let test_transform_error_permission _ =
    ...
    | Mcp_error err ->
      ...
      assert_bool "Should contain error message"
        (String.is_substring err.message ~substring:"Permission denied: test error")
    | _ -> assert_failure "Should return MCP error"
  ...
  let test_transform_error_timeout _ =
    ...
    | Mcp_error err ->
      assert_equal (-32000) err.code;
      assert_bool "Should contain error message"
        (String.is_substring err.message ~substring:"Request timeout: test error")
    | _ -> assert_failure "Should return MCP error"
  ...
  let test_on_message_error_transform _ =
    ...
    | Error (Mcp_error err) ->
      ...
      assert_bool "Should contain error message"
        (String.is_substring err.message ~substring:"Invalid params: test error");
      let has_error_log = List.exists !log_messages ~f:(fun (lvl, msg) ->
        lvl = Logger.Error && 
        String.is_substring msg ~substring:"Error in test_method: Invalid_argument: test error"
      ) in
  ...
  let suite =
    "Error handling middleware tests" >::: [
      ...
      "test transform error value error" >:: test_transform_error_value_error;
      "test transform error file not found" >:: test_transform_error_file_not_found;
      ...
      "test on message success" >:: (fun _ -> Thread_safe.block_on_async_exn test_on_message_success);
      "test on message error transform" >:: (fun _ -> Thread_safe.block_on_async_exn test_on_message_error_transform);
      "test get error stats" >:: test_get_error_stats;
    ]
end 

let () =
  run_test_tt_main Test_error_handling.suite 