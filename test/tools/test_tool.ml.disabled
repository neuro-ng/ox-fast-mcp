...
module TestTools = struct
  ...
  type tool_handler =
    execution_context -> Yojson.Safe.t -> content_type list Lwt.t
...
let%expect_test "execute tool" =
  ...
  Lwt_main.run
    (let%lwt result =
       tool.handler ctx (`Assoc [ ("x", `Int 5); ("y", `Int 3) ])
     in
     match result with
     | [ Text content ] ->
...
let%expect_test "invalid parameter type" =
  ...
  Lwt_main.run
    (try%lwt
       let%lwt _ =
         tool.handler ctx (`Assoc [ ("x", `String "not a number") ])
       in
...
let test_tool_manager_creation () =
  let manager =
    TestTools.{ tools = Hashtbl.create 16; duplicate_behavior = `Warn }
  in
...
let test_tool_registration () =
  let manager =
    TestTools.{ tools = Hashtbl.create 16; duplicate_behavior = `Error }
  in
  ...
  check string "retrieved tool description" "First tool"
    retrieved_tool1.description
...
let test_tool_removal () =
  ...
  check bool "tool exists before removal" true
    (Hashtbl.mem manager.tools "removable_tool");
...
let test_tool_mcp_serialization () =
  ...
  check string "tool description" "Tool for serialization testing"
    tool.description;
...
let test_tool_filtering () =
  ...
  let filter_by_tag tag_filter =
    Hashtbl.fold
      (fun _name (tool : TestTools.function_tool) acc ->
        if List.exists (fun tag -> tag = tag_filter) tool.tags then tool :: acc
        else acc)
      manager.tools []
  in
