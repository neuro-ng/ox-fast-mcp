...
let%expect_test "error on invalid tool parameters" =
  ...
  let tool = Tool.from_function
    ...
    ~parameters:(`String "invalid schema")  (* Should be an object *)
  in
  match%bind
    Monitor.try_with (fun () ->
      return (add_tool manager tool))
...
let%expect_test "execute tool with all args" =
  ...
  let tool = create_test_tool "add" "Add two numbers" in
  let _added_tool = add_tool manager tool in
  let%bind result = call_tool manager "add" (`Assoc [("x", `Int 5); ("y", `Int 7)]) in
  print_s [%sexp (List.hd_exn result : Content_block.t)];
  [%expect {| (Text 12) |}];
  return ()
...
let%expect_test "different key types" =
  ...
  let%bind result2 = call_tool manager "add" (`Assoc [("x", `Int 3); ("y", `Int 7)]) in
