...
(* Python imports mapping to OCaml modules:
   - from fastmcp import FastMCP -> Server module (not yet available)
   - from fastmcp.client.client import Client -> Client module (not yet available) 
   - from fastmcp.exceptions import ToolError -> Tools.Errors module
   - from fastmcp.tools import Tool, forward, forward_raw -> Tools.Tool_transform
   - from fastmcp.tools.tool import FunctionTool -> Tools.Tool_types.function_tool
   - from fastmcp.tools.tool_transform import ArgTransform, TransformedTool -> Tools.Tool_types.Arg_transform, transformed
*)
...
(* Helper to get property from tool schema - equivalent to Python get_property *)
...
let create_required_param_tool () : function_tool =
  let handler _ctx args =
    Lwt.return (
      match args with
      | `Assoc args_list ->
          let required_param = List.Assoc.find_exn args_list ~equal:String.equal "required_param" in
          let optional_param = 
...
let%expect_test "tool from tool no change" =
  ...
  print_endline (Printf.sprintf "Parameters equal: %b" 
    (Yojson.Safe.equal add_tool.base.parameters new_tool.base.parameters));
  print_endline (Printf.sprintf "Description equal: %b" 
...
(* Equivalent to async def test_renamed_arg_description_is_maintained(add_tool) *)
let%expect_test "renamed arg description is maintained" =
  let add_tool = create_add_tool () in
  let transform_args = Map.of_alist_exn (module String) [
    ("old_x", Arg_transform.create ~name:"new_x" ())
  ] in
  let new_tool = create_from_tool ~transform_args add_tool in
  
  let description = match get_property new_tool "new_x" with
    | Some (`Assoc props) ->
...
(* Equivalent to async def test_tool_defaults_are_maintained_on_unmapped_args(add_tool) *)
...
(* Equivalent to async def test_tool_defaults_are_maintained_on_mapped_args(add_tool) *)
...
let%expect_test "tool change arg name" =
  ...
  let properties = match new_tool.base.parameters with
    | `Assoc fields ->
        (match List.Assoc.find fields ~equal:String.equal "properties" with
  ...
  let required = match new_tool.base.parameters with
    | `Assoc fields ->
        (match List.Assoc.find fields ~equal:String.equal "required" with
        | Some (`List req_list) -> 
            List.filter_map req_list ~f:(function `String s -> Some s | _ -> None)
        | _ -> [])
    | _ -> []
  in
  
  print_endline (Printf.sprintf "Properties: %s" (String.concat ~sep:", " (List.sort ~compare:String.compare properties)));
  print_endline (Printf.sprintf "Required: %s" (String.concat ~sep:", " required));
  [%expect {|
...
let%expect_test "tool change arg description" =
  ...
  let description = match get_property new_tool "old_x" with
    | Some (`Assoc props) ->
        (match List.Assoc.find props ~equal:String.equal "description" with
        | Some (`String desc) -> desc
        | _ -> "no description")
...
let%expect_test "tool drop arg" =
  Lwt_main.run (
    ...
    print_endline (Printf.sprintf "Properties: %s" (String.concat ~sep:", " properties));
...
let%expect_test "dropped args error if provided" =
  ...
  let result = 
    Lwt_main.run (
      try%lwt
        let%lwt _result = new_tool.fn ctx (`Assoc [("old_x", `Int 1); ("old_y", `Int 2)]) in
        Lwt.return "No error"
      with
...
(* Equivalent to async def test_hidden_arg_without_default_uses_parent_default(add_tool) *)
...
(* Equivalent to async def test_hide_required_param_without_default_raises_error() *)
...
let%expect_test "hide required param with user default works" =
  Lwt_main.run (
    let required_tool = create_required_param_tool () in
    let transform_args = Map.of_alist_exn (module String) [
      ("required_param", Arg_transform.create ~hide:true ~default:(`Int 5) ())
    ] in
    ...
    let ctx = create_execution_context () in
    let%lwt result = new_tool.fn ctx (`Assoc [("optional_param", `Int 20)]) in
    match result with
    | [Text content] -> 
        print_endline (Printf.sprintf "Result: %s" content);
        Lwt.return_unit
...
let%expect_test "custom transform functions not yet implemented" =
  print_endline "TODO: Implement custom transform functions (transform_fn parameter)";
  ...
  print_endline "Python test: test_fn_with_kwargs_receives_transformed_arg_names";
  print_endline "Python test: test_fn_with_kwargs_handles_partial_explicit_args";
  print_endline "Python test: test_fn_with_kwargs_mixed_mapped_and_unmapped_args";
...
let%expect_test "transform args validation unknown arg" =
  ...
  in
  print_endline (Printf.sprintf "Error: %s" result);
  [%expect {| Error: Unknown arguments in transform_args: unknown_param. Parent tool has: old_x, old_y |}]
...
let%expect_test "transform args creates duplicate names" =
  ...
  in
  print_endline (Printf.sprintf "Error: %s" result);
  [%expect {| Error: Multiple arguments would be mapped to the same names: same_name |}]
...
let%expect_test "function parameter validation not yet implemented" =
  ...
  print_endline "Python test: test_function_without_kwargs_can_have_extra_params";
...
(* Equivalent to async def test_arg_transform_default_and_factory_raises_error() *)
let%expect_test "arg transform default and factory raises error" =
  let result =
    try
      let _transform = Arg_transform.create 
        ~default:(`Int 42) 
  ...
  in
  print_endline (Printf.sprintf "Error: %s" result);
  [%expect {| Error: Cannot specify both 'default' and 'default_factory' in ArgTransform |}]
...
let%expect_test "schema structure validation" =
  let add_tool = create_add_tool () in
  let transform_args = Map.of_alist_exn (module String) [
    ("old_x", Arg_transform.create ~name:"new_x" ~description:"A transformed parameter" ())
  ] in
  let new_tool = create_from_tool ~transform_args add_tool in
  
  (* Check schema structure *)
  let schema_valid = match new_tool.base.parameters with
    | `Assoc fields ->
        let has_type = List.exists fields ~f:(fun (k, v) -> 
          String.equal k "type" && match v with `String "object" -> true | _ -> false) in
        let has_properties = List.exists fields ~f:(fun (k, _) -> String.equal k "properties") in
        let has_required = List.exists fields ~f:(fun (k, _) -> String.equal k "required") in
  ...
  let additional_props = match new_tool.base.parameters with
    | `Assoc fields ->
        (match List.Assoc.find fields ~equal:String.equal "additionalProperties" with
        | Some (`Bool b) -> Some b
        | _ -> None)
    | _ -> None
  in
  
  print_endline (Printf.sprintf "AdditionalProperties: %s" 
    (match additional_props with Some b -> Bool.to_string b | None -> "not set"));
  [%expect {|
    Schema valid: true
...
let%expect_test "advanced arg transform features not yet implemented" =
  ...
  print_endline "Python test: test_arg_transform_default_factory_called_each_time";
  ...
  print_endline "Python test: test_arg_transform_required_true_with_default_raises_error";
  print_endline "Python test: test_arg_transform_required_true_with_factory_raises_error";
...
let%expect_test "arg transform type handling - various types" =
  ...
  let test_type type_name =
    ...
    let actual_type = match get_property new_tool "old_x" with
      | Some (`Assoc props) ->
...
let%expect_test "complex type system not yet implemented" =
  ...
  print_endline "Python test: test_arg_transform_precedence_over_function_without_kwargs";
  print_endline "Python test: test_arg_transform_precedence_over_function_with_kwargs";
...
let%expect_test "arg transform examples in schema" =
  let add_tool = create_add_tool () in
  let transform_args = Map.of_alist_exn (module String) [
    ("old_x", Arg_transform.create ~examples:(`List [`Int 1; `Int 2; `Int 3]) ())
  ] in
  let new_tool = create_from_tool ~transform_args add_tool in
  
  let examples = match get_property new_tool "old_x" with
    | Some (`Assoc props) ->
  ...
  print_endline (Printf.sprintf "Examples: %s" (String.concat ~sep:", " examples));
...
let%expect_test "comprehensive implementation status summary" =
  ...
  print_endline "  - Basic tool transformation (rename, hide, type change, description)";
  ...
  print_endline "  - Argument validation (unknown args, duplicates, dropped args, missing required)";
  print_endline "  - Type transformations (integer, number, string, boolean, array, object)";
  print_endline "  - Examples support in schema";
  print_endline "  - ArgTransform validation (default+factory conflicts)";
  print_endline "  - Schema structure validation and additionalProperties handling";
  print_endline "";
  ...
  print_endline "  4. Advanced ArgTransform features (default_factory, required=True)";
  print_endline "";
