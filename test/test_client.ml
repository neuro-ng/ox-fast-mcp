...
let create (server : Server.Ox_fast_mcp.t) : t Deferred.t =
  (* Simulate MCP initialization - simplified version *)
  let server_info : Mcp.Types.implementation =
    {
      version =
        Option.value (Server.Ox_fast_mcp.version server) ~default:"1.0.0";
      website_url = None;
      (* Cannot access private field from interface *)
      icons = None;
      (* Cannot access private field from interface *)
      base_metadata = { name = Server.Ox_fast_mcp.name server; title = None };
    }
  in
...
let list_tools_json (t : t) : Yojson.Safe.t list Deferred.t =
  return (Server.Ox_fast_mcp.list_tools_mcp t.server)

let list_resources_json (t : t) : Yojson.Safe.t list Deferred.t =
  return (Server.Ox_fast_mcp.list_resources_mcp t.server)
...
let call_tool (t : t) (name : string)
  ...
  Server.Ox_fast_mcp.call_tool t.server ~name ~arguments:args_json

(** Resource operations *)
let read_resource (t : t) (uri : string) : string Deferred.t =
  Server.Ox_fast_mcp.read_resource t.server ~uri

(** Prompt operations *)
let get_prompt (t : t) (name : string)
    (arguments : (string * Yojson.Safe.t) list) : Yojson.Safe.t Deferred.t =
  let args_json = `Assoc arguments in
  Server.Ox_fast_mcp.get_prompt t.server ~name ~arguments:args_json
