...
let test_jsonrpc_request_serialization () =
  ...
  | Ok parsed_req ->
    ...
    check (option string) "id" (Some "test-id") 
      (match parsed_req.id with Some (`String s) -> Some s | _ -> None)
...
let test_resource_def_creation () =
  ...
  check (option string) "resource name" (Some "Application Settings") resource.name;
...
let test_prompt_def_creation () =
  let prompt = {
    ...
    arguments = Some [
      { name = "text"; type_ = "string"; description = Some "Text to summarize"; required = true }
    ];
  } in
  check string "prompt name" "summarize" prompt.name;
  check string "prompt description" "Generate a summary of the given text" prompt.description;
...
let test_content_type_variants () =
  ...
  let resource_content = Resource { uri = "file://test.txt"; text = Some "content"; mime_type = Some "text/plain" } in
...
let test_transport_types () =
  ...
  let http_transport = Http { host = "localhost"; port = 8080; path = "/mcp" } in
...
let () =
  run "MCP Types Tests" [
    "JSON-RPC", [
      test_case "Request serialization" `Quick test_jsonrpc_request_serialization;
    ];
    "Tool definitions", [
