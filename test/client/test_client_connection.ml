...
let test_initialize_called_once switch () =
  ...
  let mock_initialize _session = 
    ...
    return (Types.Initialize_result.create ~server_info:(Types.Server_info.create ~name:"TestServer" ~version:"1.0.0" ()))
  in

  let client = Client.create ~initialize:mock_initialize (Transports.FastMCP_transport server) in
...
let test_initialize_result switch () =
  ...
  check_raises "not connected error"
    (Failure "Client is not connected")
  ...
  let%bind () = Client.with_client client (fun () ->
    ...
    check bool "has version" true (Option.is_some result.Types.server_info.version);
    return ()
  ) in
...
let test_server_info_custom_version switch () =
  ...
  let server_with_version = Server.create ~name:"CustomVersionServer" ~version:"1.2.3" () in
  let client = Client.create (Transports.FastMCP_transport server_with_version) in

  let%bind () = Client.with_client client (fun () ->
    let result = Client.initialize_result client in
    check string "server name" "CustomVersionServer" result.Types.server_info.name;
    check string "server version" "1.2.3" (Option.value_exn result.Types.server_info.version);
    return ()
  ) in

  (* Test without version *)
  let server_without_version = Server.create ~name:"DefaultVersionServer" () in
  let client = Client.create (Transports.FastMCP_transport server_without_version) in

  let%bind () = Client.with_client client (fun () ->
    let result = Client.initialize_result client in
    check string "server name" "DefaultVersionServer" result.Types.server_info.name;
    check bool "has default version" true (Option.is_some result.Types.server_info.version);
...
let test_client_nested_context_manager switch () =
  ...
  let%bind () = Client.with_client client (fun () ->
    check bool "connected in outer context" true (Client.is_connected client);
    check bool "has session in outer context" true (Option.is_some client.session);
    let session = client.session in

    let%bind () = Client.with_client client (fun () ->
      check bool "connected in inner context 1" true (Client.is_connected client);
      check bool "same session in inner context 1" true (Option.equal phys_equal client.session session);
      return ()
    ) in

    let%bind () = Client.with_client client (fun () ->
      check bool "connected in inner context 2" true (Client.is_connected client);
      check bool "same session in inner context 2" true (Option.equal phys_equal client.session session);
      return ()
    ) in
    return ()
  ) in
...
let test_concurrent_client_context_managers switch () =
  ...
  (* Add a tool *)
  let echo_tool = Tool.create ~name:"echo" ~f:(fun args ->
    let text = List.Assoc.find_exn args ~equal:String.equal "text" |> Yojson.Safe.to_string in
  ...
  Hashtbl.iter results ~f:(fun count ->
    check int "tool count" 1 count
  );
  return ()
...
let test_timeout_tool_call_overrides_client_timeout switch () =
  ...
  (* Add a sleep tool *)
  let sleep_tool = Tool.create ~name:"sleep" ~f:(fun args ->
    let seconds = List.Assoc.find_exn args ~equal:String.equal "seconds" |> Yojson.Safe.to_float in
  ...
  let client = Client.create ~timeout:(Time.Span.of_sec 2.0) (Transports.FastMCP_transport server) in

  let%bind () = Client.with_client client (fun () ->
    match%bind Client.call_tool ~timeout:(Time.Span.of_sec 0.01) client ~name:"sleep" ~arguments:[("seconds", `Float 0.1)] () with
    | Ok _ -> failwith "Expected timeout error but got success"
    | Error e ->
      check bool "error contains timeout message" true
        (String.is_substring ~substring:"Timed out while waiting for response" (Error.to_string_hum e));
      return ()
  ) in
  return ()
...
let test_timeout_tool_call_overrides_client_timeout_even_if_lower switch () =
  ...
  let client = Client.create ~timeout:(Time.Span.of_sec 0.01) (Transports.FastMCP_transport server) in

  let%bind () = Client.with_client client (fun () ->
    let%bind result = Client.call_tool ~timeout:(Time.Span.of_sec 2.0) client ~name:"sleep" ~arguments:[("seconds", `Float 0.1)] () in
    match result with
    | Ok content ->
      check bool "completed successfully" true
        (match List.hd_exn content with
         | Types.Text_content {text; _} -> String.is_substring ~substring:"Slept for 0.1 seconds" text
         | _ -> false);
      return ()
...
let () =
  run "Client Connection Tests" [
    "connection", [
      ...
      test_case "server info custom version" `Quick test_server_info_custom_version;
      test_case "nested context manager" `Quick test_client_nested_context_manager;
      test_case "concurrent client usage" `Quick test_concurrent_client_context_managers;
      test_case "timeout tool call overrides client timeout" `Quick test_timeout_tool_call_overrides_client_timeout;
      test_case "timeout tool call overrides client timeout even if lower" `Quick test_timeout_tool_call_overrides_client_timeout_even_if_lower;
    ];
  ] 