...
let create_tagged_resources_server () =
  ...
  let tagged_resource = Resource.create 
    ~uri:"data://tagged"
    ~tags:["test"; "metadata"]
    ~description:"A tagged resource"
    ~f:(fun () -> return (`Assoc [("type", `String "tagged_data")]))
  in
  Server.add_resource server tagged_resource;

  (* Add a resource template with tags *)
  let tagged_template = Resource_template.create
    ~uri_template:"template://{id}"
    ~tags:["template"; "parameterized"]
    ~description:"A tagged template"
    ~f:(fun ~id ->
      return (`Assoc [
        ("id", `String id);
        ("type", `String "template_data")
      ])
    )
  in
...
let test_tagged_resource_metadata switch () =
  ...
  let%bind () = Client.with_client client (fun () ->
    ...
    check string "resource uri" "data://tagged" (Uri.to_string resource.Types.uri);
    check string "resource description" "A tagged resource" (Option.value_exn resource.Types.description);
    return ()
  ) in
  return ()
...
let test_tagged_template_metadata switch () =
  ...
  let%bind () = Client.with_client client (fun () ->
    ...
    check string "template uri" "template://{id}" template.Types.uri_template;
    check string "template description" "A tagged template" (Option.value_exn template.Types.description);
    return ()
  ) in
  return ()
...
let test_tagged_template_functionality switch () =
  ...
  let%bind () = Client.with_client client (fun () ->
    let%bind result = Client.read_resource client (Uri.of_string "template://123") in
    match result with
    | Ok content ->
...
let () =
  run "Client Tagged Tests" [
    "tagged resources", [
      test_case "tagged resource metadata" `Quick test_tagged_resource_metadata;
      test_case "tagged template metadata" `Quick test_tagged_template_metadata;
      test_case "tagged template functionality" `Quick test_tagged_template_functionality;
    ];
  ] 