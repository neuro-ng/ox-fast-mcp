...
let%expect_test "success_html contains expected content" =
  ...
  let has_title =
    String.is_substring html ~substring:"Authorization Successful"
  in
...
let%expect_test "error_html contains error message" =
  let html =
    Client_auth.Oauth_callback.error_html ~error:"Test error message"
  in
...
(* Full integration tests require: 1. Actually starting the HTTP server on a
   port 2. Making HTTP requests to the callback endpoint 3. This is more
   suitable for integration test suite

   The start_callback_server function is tested via manual verification: - Run
   server manually - curl http://localhost:PORT/callback?code=test&state=xyz -
   Verify HTML response and Ivar fill

   Example manual test: ```ocaml let () = let open Async in don't_wait_for (
   let%bind result = Client_auth.Oauth_callback.start_callback_server ~port:8765
   ~timeout:(Time_ns.Span.of_sec 30.0) in printf "Got code: %s\n" (Option.value
   result.code ~default:"none"); return () ); Scheduler.go () ``` *)
