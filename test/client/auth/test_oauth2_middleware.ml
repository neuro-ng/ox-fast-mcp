(** OAuth2 Middleware Tests
    ...
    Tests for oauth2_middleware.ml functionality including header application,
    token refresh, and 401/403 handling. *)
...
let%expect_test "create_middleware returns valid state" =
  ...
  in
  let _middleware_state =
    Mcp_client_auth.Oauth2_middleware.create_middleware oauth
  in
...
let%expect_test "apply_oauth_auth adds authorization header when token exists" =
  ...
  let oauth =
    Mcp_client_auth.Oauth2.create ~server_url:"http://localhost:9999"
      ~client_metadata:metadata
      ~storage:
        (Mcp_client_auth.Oauth2.Storage
           ((module Headless_oauth.Test_storage), storage))
      ()
  in
  ...
  let result =
    Mcp_client_auth.Oauth2_middleware.apply_oauth_auth oauth original_headers
  in

  (* Check that Authorization header was added *)
  let has_auth =
    List.exists result ~f:(fun (k, _) ->
        String.equal (String.lowercase k) "authorization")
  in
  let auth_value =
    List.find_map result ~f:(fun (k, v) ->
        if String.equal (String.lowercase k) "authorization" then Some v
        else None)
  in
...
let%expect_test "wrap_client_with_oauth creates wrapped client" =
  ...
  let _wrapped =
    Mcp_client_auth.Oauth2_middleware.wrap_client_with_oauth oauth
      mock_http_client
  in
...
let%expect_test "middleware_handles_401_with_oauth_flow" =
  ...
  let%bind response, body =
    Mcp_client_auth.Oauth2_middleware.oauth_middleware middleware_state
      mock_http_client request
  in
...
let%expect_test "middleware_refreshes_expired_token" =
  ...
  (* Set an "expired" token (expires_in = 0 means already expired with 5s
     buffer) *)
  let expired_token =
    Mcp_shared.Auth.
      {
  ...
  (* Note: Token appears valid because calculate_token_expiry subtracts 5s from
     expires_in, so 0 results in a time slightly in the past but recent enough
     to sometimes pass. Can't refresh without client_info. *)
  [%expect {|
