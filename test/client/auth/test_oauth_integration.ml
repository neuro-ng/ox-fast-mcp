(** OAuth Integration Tests
    ...
    End-to-end tests using HeadlessOAuth and MockOAuthServer. These tests verify
    the full OAuth flow without browser interaction. *)
...
let%expect_test "headless oauth creates valid oauth client" =
  let oauth =
    Headless_oauth.create_with_defaults ~server_url:"http://localhost:9999"
...
let%expect_test "headless oauth with custom metadata" =
  let headless =
    Headless_oauth.create ~server_url:"http://localhost:8888"
      ~mock_auth_code:"custom_code" ~mock_state:"custom_state" ()
  in
  let metadata =
    Headless_oauth.default_client_metadata
      ~scopes:[ "read"; "write"; "admin" ]
      ~client_name:"Integration Test" ()
  in
  let _oauth =
    Headless_oauth.create_oauth_client headless ~client_metadata:metadata
  in
...
let%expect_test "mock oauth server create and configure" =
  ...
  Mock_oauth_server.add_valid_code server ~code:"auth_code_123"
...
let%expect_test "file token storage hash generation" =
  ...
  let storage1 =
    Mcp_client_auth.File_token_storage.create_default
...
(* Note: These tests start an actual HTTP server on localhost. Each test uses a
...
let%expect_test "oauth_metadata_discovery_with_mock_server" =
  ...
  let metadata_url =
    sprintf "%s/.well-known/oauth-authorization-server" issuer
  in
  ...
  let issuer_field =
    Yojson.Safe.Util.member "issuer" json |> Yojson.Safe.Util.to_string
  in
  let token_endpoint =
    Yojson.Safe.Util.member "token_endpoint" json |> Yojson.Safe.Util.to_string
  in
  let auth_endpoint =
    Yojson.Safe.Util.member "authorization_endpoint" json
...
let%expect_test "client_registration_with_mock_server" =
  ...
  in
  let%bind resp, body =
    Cohttp_async.Client.post ~headers ~body:(`String body)
  ...
  let client_id =
    Yojson.Safe.Util.member "client_id" json |> Yojson.Safe.Util.to_string
  in
  let client_secret =
    Yojson.Safe.Util.member "client_secret" json |> Yojson.Safe.Util.to_string
  in
...
let%expect_test "token_exchange_with_mock_server" =
  ...
  Mock_oauth_server.add_valid_code server ~code:"test_auth_code_123"
    ~client_id:"test_client";
  ...
  let access_token =
    Yojson.Safe.Util.member "access_token" json |> Yojson.Safe.Util.to_string
  in
  let token_type =
    Yojson.Safe.Util.member "token_type" json |> Yojson.Safe.Util.to_string
  in
  let has_refresh =
    match Yojson.Safe.Util.member "refresh_token" json with
    | `Null -> false
    | _ -> true
  in
...
let%expect_test "token_refresh_with_mock_server" =
  ...
  Mock_oauth_server.add_valid_code server ~code:"initial_code"
    ~client_id:"test_client";
  ...
  let json = Yojson.Safe.from_string body_str in
  let refresh_token =
    Yojson.Safe.Util.member "refresh_token" json |> Yojson.Safe.Util.to_string
  in
  ...
  in
  let%bind resp, body =
    Cohttp_async.Client.post ~headers ~body:(`String refresh_body)
      (Uri.of_string token_url)
  in
  let status = Cohttp.Response.status resp in
  let%bind body_str = Cohttp_async.Body.to_string body in
...
let%expect_test "full_oauth_flow_headless_with_mock_server" =
  ...
  Mock_oauth_server.add_valid_code server ~code:auth_code
  ...
  let headless =
    Headless_oauth.create ~server_url:issuer ~mock_auth_code:auth_code
      ~mock_state:"test_state_xyz" ()
  in

  let metadata =
    Headless_oauth.default_client_metadata ~scopes:[ "read"; "write" ]
