...
module Error_test_server = struct
  let create_with_error_tool () =
    ...
    let error_tool = Tool.create ~name:"error_tool" ~f:(fun _args ->
  ...
  let create_with_custom_error_tool () =
    ...
    let custom_error_tool = Tool.create ~name:"custom_error_tool" ~f:(fun _args ->
  ...
  let create_with_error_resource () =
    ...
    let error_resource = Resource.create ~uri:"exception://resource" ~f:(fun () ->
  ...
  let create_with_custom_error_resource () =
    ...
    let error_resource = Resource.create ~uri:"error://resource" ~f:(fun () ->
  ...
  let create_with_error_template () =
    ...
    let error_template = Resource_template.create ~uri_template:"exception://resource/{id}" ~f:(fun ~id:_ ->
      raise (Failure "This is an internal error (sensitive)")
    ) in
  ...
  let create_with_custom_error_template () =
    ...
    let error_template = Resource_template.create ~uri_template:"error://resource/{id}" ~f:(fun ~id:_ ->
      raise (Resource_error.Resource_error "This is a resource error (xyz)")
    ) in
    Server.add_resource_template server error_template;
...
let test_general_tool_exceptions switch () =
  ...
  let%bind () = Client.with_client client (fun () ->
    let%bind result = Client.call_tool_mcp client ~name:"error_tool" ~arguments:[] () in
...
let test_custom_tool_errors switch () =
  ...
  let%bind () = Client.with_client client (fun () ->
    let%bind result = Client.call_tool_mcp client ~name:"custom_error_tool" ~arguments:[] () in
    check bool "is error" true result.Types.is_error;
    check bool "error contains message" true
      (match List.hd_exn result.Types.content with
       | Types.Text_content {text; _} -> 
...
let test_general_resource_exceptions switch () =
  ...
  let%bind () = Client.with_client client (fun () ->
    match%bind Client.read_resource client (Uri.of_string "exception://resource") with
    | Ok _ -> failwith "Expected error but got success"
    | Error e ->
      check bool "error contains message" true
        (String.is_substring ~substring:"Error reading resource" (Error.to_string_hum e) &&
         String.is_substring ~substring:"sensitive" (Error.to_string_hum e) &&
         String.is_substring ~substring:"internal error" (Error.to_string_hum e));
      return ()
  ) in
  return ()
...
let test_custom_resource_errors switch () =
  ...
  let%bind () = Client.with_client client (fun () ->
    match%bind Client.read_resource client (Uri.of_string "error://resource") with
    | Ok _ -> failwith "Expected error but got success"
    | Error e ->
      check bool "error contains message" true
        (String.is_substring ~substring:"This is a resource error (xyz)" (Error.to_string_hum e));
      return ()
  ) in
  return ()
...
let test_general_template_exceptions switch () =
  ...
  let%bind () = Client.with_client client (fun () ->
    match%bind Client.read_resource client (Uri.of_string "exception://resource/123") with
    | Ok _ -> failwith "Expected error but got success"
    | Error e ->
      check bool "error contains message" true
        (String.is_substring ~substring:"Error reading resource" (Error.to_string_hum e) &&
...
let test_custom_template_errors switch () =
  ...
  let%bind () = Client.with_client client (fun () ->
    match%bind Client.read_resource client (Uri.of_string "error://resource/123") with
    | Ok _ -> failwith "Expected error but got success"
    | Error e ->
      check bool "error contains message" true
        (String.is_substring ~substring:"This is a resource error (xyz)" (Error.to_string_hum e));
...
let test_general_resource_exceptions_are_masked_when_enabled switch () =
  ...
  let%bind () = Client.with_client client (fun () ->
    ...
    | Error e ->
      check bool "error details are masked" true
        (String.is_substring ~substring:"Error reading resource" (Error.to_string_hum e) &&
         not (String.is_substring ~substring:"sensitive" (Error.to_string_hum e)) &&
         not (String.is_substring ~substring:"internal error" (Error.to_string_hum e)));
      return ()
  ) in
  return ()
...
let test_timeout switch () =
  ...
  let client = Client.create ~timeout:(Time.Span.of_sec 0.05) (Transports.FastMCP_transport server) in

  let%bind () = Client.with_client client (fun () ->
    match%bind Client.call_tool client ~name:"sleep" ~arguments:[("seconds", `Float 0.1)] () with
...
let () =
  run "Client Error Tests" [
    "error handling", [
      test_case "general tool exceptions" `Quick test_general_tool_exceptions;
      test_case "custom tool errors" `Quick test_custom_tool_errors;
      test_case "general resource exceptions" `Quick test_general_resource_exceptions;
      test_case "custom resource errors" `Quick test_custom_resource_errors;
      test_case "general template exceptions" `Quick test_general_template_exceptions;
      test_case "custom template errors" `Quick test_custom_template_errors;
      test_case "general tool exceptions are masked" `Quick test_general_tool_exceptions_are_masked_when_enabled;
      test_case "general resource exceptions are masked" `Quick test_general_resource_exceptions_are_masked_when_enabled;
      test_case "general template exceptions are masked" `Quick test_general_template_exceptions_are_masked_when_enabled;
    ];
    "timeouts", [
