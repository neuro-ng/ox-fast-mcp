...
let test_prompt_creation () =
  ...
  let handler _ctx _args =
    ...
    Lwt.return [
      { role = "user"; content = create_text_content "Please summarize this text" };
      { role = "assistant"; content = create_text_content "Here's a summary..." }
    ]
  in
  
  let prompt = TestPrompts.{
    name;
    description;
    arguments;
    handler;
...
let test_prompt_execution () =
  Lwt_main.run (
    ...
    let args = [("text", `String "This is a long text that needs summarization")] in
    
    let handler _ctx _args =
      let* () = Lwt.return_unit in
      Lwt.return [
        { role = "user"; content = create_text_content "Summarize: This is a long text" };
        { role = "assistant"; content = create_text_content "Summary: A text." }
      ]
    in
    ...
    check bool "first message is text" true 
      (match first_result.content with Text _ -> true | _ -> false);
...
let test_prompt_registration () =
  ...
  let prompt2 = TestPrompts.{
    name = "prompt2";
    description = "Second prompt";
    arguments = Some [{ name = "input"; description = None; required = true }];
    handler = (fun _ctx _args -> Lwt.return []);
    enabled = true;
    tags = ["tag1"];
  } in
  ...
  check string "retrieved prompt description" "First prompt" retrieved_prompt1.description
...
let test_prompt_argument_validation () =
  ...
  let validate_prompt_arguments (args : prompt_argument list) =
    List.for_all (fun (arg : prompt_argument) -> String.length arg.name > 0) args
  in
  
  check bool "valid args accepted" true (validate_prompt_arguments valid_args);
  check bool "invalid args rejected" false (validate_prompt_arguments invalid_args)
...
let test_prompt_mcp_serialization () =
  let prompt = TestPrompts.{
    ...
    arguments = Some [{ name = "input"; description = Some "Input text"; required = true }];
    handler = (fun _ctx _args -> Lwt.return []);
    enabled = true;
    tags = ["test"];
  } in
  ...
  check string "mcp prompt description" "Prompt for serialization testing" mcp_prompt.description;
  check bool "mcp prompt has arguments" true (Option.is_some mcp_prompt.arguments)
  ...
  Lwt_main.run (
    ...
    let template_handler _ctx args =
      let name = match List.assoc_opt "name" args with
        ...
        | _ -> "Unknown"
      in
      let age = match List.assoc_opt "age" args with
        | Some (`String a) -> a
        | _ -> "Unknown"
      in
      ...
      let* () = Lwt.return_unit in
      Lwt.return [
        { role = "system"; content = create_text_content "You are a helpful assistant." };
        { role = "user"; content = create_text_content ("Tell me about " ^ name ^ " who is " ^ age ^ " years old.") }
      ]
    in
    ...
    let prompt = TestPrompts.{
      ...
      arguments = Some [
        { name = "name"; description = Some "Person's name"; required = true };
        { name = "age"; description = Some "Person's age"; required = true }
      ];
  ...
  Lwt_main.run (
    ...
    let* result = 
      Lwt.catch 
        ...
        (function
         | Failure msg -> Lwt.return [{ role = "error"; content = create_text_content ("Error: " ^ msg) }]
         | exn -> Lwt.fail exn)
    in
...
let test_prompt_filtering () =
  ...
  let filter_by_tag tag_filter =
    Hashtbl.fold (fun _name (prompt : TestPrompts.function_prompt) acc ->
...
let test_complex_prompt_messages () =
  Lwt_main.run (
    ...
    let complex_handler _ctx args =
      let mode = match List.assoc_opt "mode" args with
        | Some (`String m) -> m
        | _ -> "text"
      in
      ...
      match mode with
      | "text" -> Lwt.return [
          { role = "user"; content = create_text_content "Simple text prompt" }
        ]
      | "mixed" -> Lwt.return [
          { role = "system"; content = create_text_content "System instructions" };
          { role = "user"; content = create_text_content "User query" };
          { role = "assistant"; content = create_image_content ~data:"base64data" ~mime_type:"image/png" () };
        ]
      | _ -> Lwt.return []
    in
    ...
    check bool "system content is text" true 
      (match system_msg.content with Text _ -> true | _ -> false);
    check bool "user content is text" true 
      (match user_msg.content with Text _ -> true | _ -> false);
    check bool "assistant content is image" true 
      (match assistant_msg.content with Image _ -> true | _ -> false);
...
let () =
  run "Prompts Module Tests" [
    ...
    ("Prompt Execution", [
      ...
      test_case "Complex prompt messages" `Quick test_complex_prompt_messages;
    ]);
    ("Prompt Manager", [
      test_case "Prompt manager creation" `Quick test_prompt_manager_creation;
