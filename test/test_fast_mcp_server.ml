...
let test_server_with_instructions () =
  let instructions = "This server provides mathematical operations and data analysis tools." in
...
let test_tool_registration () =
  ...
  check int "tool count after registration" 1 (Fast_mcp_server.tool_count server);
...
let test_tool_with_complex_params () =
  ...
  let complex_tool ~name ~age ~hobbies = 
    Printf.sprintf "User %s is %d years old and likes: %s" 
      name age (String.concat ", " hobbies)
  in
  ...
  check bool "complex tool exists" true (Fast_mcp_server.has_tool server "create_profile")
...
let test_resource_registration () =
  ...
  check bool "resource exists" true (Fast_mcp_server.has_resource server "config://settings")
...
let test_prompt_registration () =
  ...
  Fast_mcp_server.register_prompt server
    ...
    ~func:summarize_prompt;
  
  check int "prompt count" 1 (Fast_mcp_server.prompt_count server);
  check bool "prompt exists" true (Fast_mcp_server.has_prompt server "summarize")
...
let test_tool_execution () =
  Lwt_main.run (
    ...
    (* Execute the tool *)
    let* result = Fast_mcp_server.call_tool server "multiply" 
...
let test_server_capabilities () =
  ...
  check bool "has resources capability" true (Option.is_some capabilities.resources);
...
let test_context_injection () =
  Lwt_main.run (
    ...
    let* result = Fast_mcp_server.call_tool_with_context server ctx 
      "context_tool" (`String "test_value") in
...
let () =
  run "FastMCP Server Tests" [
    "Server Creation", [
      ...
      test_case "Server with instructions" `Quick test_server_with_instructions;
    ];
    "Tool Registration", [
      test_case "Simple tool registration" `Quick test_tool_registration;
      test_case "Complex parameter tool" `Quick test_tool_with_complex_params;
    ];
    "Resource Registration", [
      test_case "Static resource registration" `Quick test_resource_registration;
      test_case "Resource template registration" `Quick test_resource_template;
    ];
    "Prompt Registration", [
