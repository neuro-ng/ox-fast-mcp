...
let test_client_resource_access () =
  Lwt_main.run (
    ...
    Fast_mcp_server.register_resource server
      ...
      ~func:(fun () -> `Assoc [("status", `String "running"); ("uptime", `Int 3600)]);
    ...
    match result with
    | Ok (Text content) ->
      check bool "resource content present" true (String.contains content "running");
      let* () = Mcp_client.close client in
      Lwt.return_unit
    | Ok _ -> fail "Expected text result"
...
let test_client_connection_lifecycle () =
  Lwt_main.run (
    ...
    (* Close connection *)
    let* () = Mcp_client.close client in
    check bool "disconnected after close" false (Mcp_client.is_connected client);
...
let test_client_concurrent_operations () =
  Lwt_main.run (
    ...
    for i = 1 to 5 do
      ...
      Fast_mcp_server.register_tool server
        ~name
        ~description:(Printf.sprintf "Tool number %d" i)
        ~func:(fun _ -> `Int i);
    done;
...
let () =
  run "MCP Client Tests" [
    ...
    "Communication", [
      test_case "Server communication" `Quick test_client_server_communication;
    ...
    "Connection Management", [
      test_case "Connection lifecycle" `Quick test_client_connection_lifecycle;
      test_case "Concurrent operations" `Quick test_client_concurrent_operations;
    ];
  ] 